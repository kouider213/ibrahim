...
const RULES = [
  ...
  { id: 4, rule: 'Kouider préfère les réponses courtes et directes',                cat: 'COMM',       priority: 7, col: '#3b82f6' },
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <Panel title="SYSTÈME BACKEND">
          <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
            ...
            <StatusPill label="UPTIME" val={fmtUptime(health?.uptime)} ok={!!health} col="#3b82f6" />
          </div>
          {nexusOk && nexus && (
            <div style={{
          ...
          <button onClick={() => void checkHealth()} disabled={loadingH} style={actionBtn('#3b82f6')}>
            ...
            {jobs.map(j => (
              <div key={j.name} style={{
                ...
                <button
                  ...
                  style={{
                    background: '#3b82f618', border: '1px solid #3b82f644', borderRadius: 6,
                    width: 28, height: 28, cursor: 'pointer', color: '#3b82f6',
        ...
        <Panel title="RÈGLES APPRISES (PHASE 8)">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
            {RULES.map(r => (
              <div key={r.id} style={{
                background: 'rgba(59,130,246,0.03)', borderRadius: 8,
        ...
        <Panel title="CAPACITÉS & AGENTS DZARYX">
          <button
            ...
            style={{ width: '100%', textAlign: 'left', background: 'rgba(59,130,246,0.06)', border: '1px solid #3b82f622', borderRadius: 8, padding: '10px 12px', cursor: 'pointer', color: '#cfefff', fontFamily: 'Inter, sans-serif', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
          >
            <span>🤖 14 agents · ce que Dzaryx sait faire</span>
            <span style={{ color: '#3b82f688' }}>{showCaps ? '▲' : '▼'}</span>
          </button>
          {showCaps && (
            <div style={{ marginTop: 10, height: 520, borderRadius: 10, overflow: 'hidden', border: '1px solid #3b82f612' }}>
              <CapacitesScreen />
            </div>
          )}
        </Panel>
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ background: 'rgba(59,130,246,0.03)', borderRadius: 12, padding: '12px', border: '1px solid #3b82f612' }}>
      <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 7, color: '#3b82f655', letterSpacing: '0.3em', marginBottom: 10 }}>{title}</div>
      {children}
    </div>
  );
}
