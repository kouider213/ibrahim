...
const SCORE_COL: Record<string, string> = {
  VIP: '#ffd700', FREQUENT: '#6366f1', FRÉQUENT: '#6366f1',
...
export default function RevenueScreen({ actor = 'kouider' }: { actor?: string }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#020810', color: '#fff', fontFamily: 'Inter, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Header */}
      <div style={{ padding: '8px 14px 6px', borderBottom: '1px solid #6366f112', flexShrink: 0, background: 'rgba(2,8,16,0.97)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          ...
          <div style={{ flex: 1, textAlign: 'center' }}>
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 9, color: '#6366f1', letterSpacing: '0.15em' }}>
              {MONTH_FR[viewMonth - 1].toUpperCase()} {viewYear}
            </span>
            {!isAtNow && (
              <button onClick={goToNow} style={{ marginLeft: 8, background: '#6366f118', border: '1px solid #6366f144', borderRadius: 4,
                fontFamily: 'Inter, sans-serif', fontSize: 6, color: '#6366f1aa', cursor: 'pointer', padding: '2px 6px' }}>
                ↩ CE MOIS
              </button>
            )}
          </div>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {([
            ...
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              border: `1px solid ${tab === t.id ? '#6366f155' : '#ffffff0f'}`,
              fontFamily: 'Inter, sans-serif', fontSize: 6, color: tab === t.id ? '#6366f1' : '#ffffff33',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {report?.dzd && (isHouari || report.dzd.bookings > 0) && (
          <div style={{ borderRadius: 12, border: '1px solid #6366f144', background: 'linear-gradient(135deg, rgba(99,102,241,0.10), rgba(2,8,16,0.6))', padding: '11px 13px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 8, color: '#6366f1', letterSpacing: '0.15em' }}>💱 CA EN DINARS (HOUARI)</span>
              <span style={{ fontSize: 7, color: '#6366f199' }}>{report.dzd.bookings} location{report.dzd.bookings > 1 ? 's' : ''}</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6 }}>
              {[
                { l: 'CA', v: report.dzd.ca, c: '#6366f1' },
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: 30, fontFamily: 'Inter, sans-serif', fontSize: 9, color: '#6366f133', letterSpacing: '0.25em' }}>
            CHARGEMENT…
          </div>
        ) : tab === 'overview' ? (
          <OverviewTab
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => void load()} style={{ ...aBtn('#6366f1'), flex: 1 }}>↻ ACTUALISER</button>
          {isCurrentMonth && (
...
function OverviewTab({ rev, report, caMonth, profitK, caHouari, nbBookings, encaisse, aEncaisser, isCurrentMonth, fmt }: {
  ...
  return (
    <>
      {/* CA + variation */}
      <div style={{ textAlign: 'center', background: 'rgba(99,102,241,0.05)', borderRadius: 12, padding: '14px', border: '1px solid #6366f11a' }}>
        <div style={{ fontSize: 7, color: '#6366f155', letterSpacing: '0.25em', fontFamily: 'Inter, sans-serif', marginBottom: 4 }}>CA TOTAL</div>
        <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 32, color: '#6366f1', textShadow: '0 0 20px #6366f155' }}>
          {fmt(caMonth)}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
        <StatMini label="RÉSAS" val={String(nbBookings)} col="#6366f1" />
...
function DetailTab({ report, fmt }: { report: FinancialReport | null; fmt: (n: number | null | undefined) => string }) {
  ...
  const STATUS_COL: Record<string, string> = {
    CONFIRMED: '#00e676', PENDING: '#ffb347', ACTIVE: '#6366f1',
  ...
  return (
    <>
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', borderRadius: 10, padding: '10px', border: '1px solid #ffffff08',
        ...
        <InfoChip label="Résas" val={String(report.totalBookings)} col="#6366f1" />
            ...
            <div key={b.id} style={{ borderRadius: 10, border: `1px solid ${sc}1a`, background: `${sc}05`, padding: '8px 10px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {b.client_price_per_day != null && (
                  <span style={{ fontSize: 7, color: '#6366f166' }}>{b.client_price_per_day}€/j</span>
                )}
...
function AnnualTab({ report, year, fmt }: {
  ...
  return (
    <>
      {/* Year header */}
      <div style={{ textAlign: 'center', fontFamily: 'Inter, sans-serif', fontSize: 9, color: '#6366f155', letterSpacing: '0.3em', marginBottom: 2 }}>
        BILAN {year}
      </div>

      {/* Big annual CA */}
      <div style={{ textAlign: 'center', background: 'rgba(99,102,241,0.05)', borderRadius: 12, padding: '14px', border: '1px solid #6366f11a' }}>
        <div style={{ fontSize: 7, color: '#6366f144', letterSpacing: '0.25em', fontFamily: 'Inter, sans-serif', marginBottom: 4 }}>CA ANNUEL TOTAL</div>
        <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 34, color: '#6366f1', textShadow: '0 0 20px #6366f155', lineHeight: 1 }}>
          {fmt(report.grossCA)}
        </div>
          ...
          {vehicles.map(([name, stats]) => (
            <div key={name} style={{ marginBottom: 9 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2, alignItems: 'center' }}>
                ...
                <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 9, color: '#6366f1', flexShrink: 0, marginLeft: 8 }}>
                  {fmt(stats.ca)}
                </span>
              </div>
              <div style={{ height: 4, background: '#ffffff08', borderRadius: 2, overflow: 'hidden', marginBottom: 2 }}>
                <div style={{ height: '100%', width: `${(stats.ca / maxCA) * 100}%`,
                  background: 'linear-gradient(90deg, #6366f155, #6366f1)', borderRadius: 2 }} />
              </div>
      ...
      {vehicles.map(([name, stats]) => (
        <div key={name} style={{ marginBottom: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
            ...
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 8, color: '#6366f1', flexShrink: 0, marginLeft: 8 }}>{fmt(stats.ca)}</span>
          </div>
          <div style={{ height: 3, background: '#ffffff0a', borderRadius: 2, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${(stats.ca / maxCA) * 100}%`, background: 'linear-gradient(90deg, #6366f188, #6366f1)', borderRadius: 2 }} />
          </div>
...
const navBtn: React.CSSProperties = {
  ...
  width: 26, height: 26, cursor: 'pointer', color: '#6366f1aa',
