...
const inp: React.CSSProperties = {
  ...
  border: '1px solid #00d4ff1a', borderRadius: 7, padding: '7px 9px',
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#020810', color: '#fff', fontFamily: 'Share Tech Mono', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '10px 14px 8px', borderBottom: '1px solid #00d4ff12', flexShrink: 0, background: 'rgba(2,8,16,0.97)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
          <div style={{ fontFamily: 'Orbitron', fontSize: 11, color: '#00d4ff', letterSpacing: '0.3em', fontWeight: 700, textShadow: '0 0 12px #00d4ff55' }}>DEMANDES</div>
          <button onClick={() => setShow(s => !s)} style={{ background: '#00d4ff18', border: '1px solid #00d4ff55', borderRadius: 8, padding: '4px 10px', fontFamily: 'Orbitron', fontSize: 7, color: '#00d4ff', cursor: 'pointer', letterSpacing: '0.12em' }}>{show ? '✕' : '+ DEMANDE'}</button>
        </div>
        <div style={{ display: 'flex', gap: 5, marginBottom: 8 }}>
          <Kpi label="NOUVEAUX" val={String(nouveaux)} col="#00d4ff" />
        ...
        <div style={{ display: 'flex', gap: 5 }}>
          {([['actifs', 'ACTIFS'], ['conclu', 'CONCLUS'], ['all', 'TOUT']] as [string, string][]).map(([k, lbl]) => (
            <button key={k} onClick={() => setFilter(k)} style={{ flex: 1, padding: '5px', borderRadius: 6, fontFamily: 'Orbitron', fontSize: 7, cursor: 'pointer', background: filter === k ? 'rgba(0,212,255,0.15)' : 'transparent', border: `1px solid #00d4ff${filter === k ? '88' : '22'}`, color: `#00d4ff${filter === k ? '' : '66'}` }}>{lbl}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        {show && (
          <div style={{ background: 'rgba(0,212,255,0.05)', border: '1px solid #00d4ff22', borderRadius: 10, padding: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {loading ? <Loader /> : filtered.length === 0 ? <Empty text="Aucune demande" /> : filtered.map(l => {
          ...
          return (
            <div key={l.id} style={{ borderRadius: 10, border: `1px solid ${st.col}33`, background: `linear-gradient(135deg, ${m.col}07, rgba(2,8,16,0.6))`, padding: '9px 12px' }}>
...
function Loader() { return <div style={{ textAlign: 'center', padding: 30, fontSize: 9, color: '#00d4ff33', fontFamily: 'Orbitron', letterSpacing: '0.25em' }}>CHARGEMENT…</div>; }
