...
export type IntentType =
  ...
  | 'app_launch'          // open local app (Chrome, VS Code, Spotify…)
...
export interface DetectedIntent {
  ...
  sysPriority?: boolean;  // true → system keyword detected, block web fallback
}
...
function hasSysPriority(cmd: string): boolean {
  const lower = cmd.toLowerCase();
  return _SYS_KEYWORDS.some(kw => lower.includes(kw));
}

function matchedSysKeywords(cmd: string): string[] {
  const lower = cmd.toLowerCase();
...
function resolveAppFromVerb(verbRe: RegExp, cmd: string): string | undefined {
...
export function detectIntent(cmd: string): DetectedIntent {
  ...
  if (fileReadM) {
    ...
    const path = resolvePath(raw) ?? raw;
    if (path) return { type: 'file_read', confidence: 0.85, sysPriority: true, args: { path } };
  }
  ...
  if (fileOpenM) {
    ...
    const path = resolvePath(raw) ?? raw;
    if (path) return { type: 'file_open', confidence: 0.82, sysPriority: true, args: { path } };
  }
  ...
  if (fileSearchM) {
    ...
    if (query) return { type: 'file_search', confidence: 0.85, sysPriority: true, args: { query } };
  }
...
interface IntentResult { message: string; ok: boolean; jobId?: string; toolUsed?: string; }
...
async function executeIntent(intent: DetectedIntent, rawCmd: string): Promise<IntentResult> {
  ...
  switch (type) {
    case 'screenshot': {
      ...
      if (!r.ok) return { ok: false, toolUsed: 'nexus:screenshot', message: `❌ Screenshot échoué : ${r.error ?? 'erreur inconnue'}` };
    ...
    case 'screen_understand': {
      ...
      if (!r.ok) return { ok: false, toolUsed: 'nexus:screen_understand', message: `❌ Analyse écran échouée : ${r.error ?? 'erreur inconnue'}` };
      const analysis = (r['analysis'] as string | undefined)?.trim();
      if (analysis) return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:screen_understand', message: `👁️ *Analyse de l'écran* :\n\n${analysis}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:screen_understand', message: `👁️ Analyse terminée — envoyée sur Telegram.` };
    }

    case 'app_launch': {
      const app = args['app'] as string;
      const r = await nexusAppLaunch(app, 20_000);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:app_launch', message: `❌ Lancement ${app} échoué : ${r.error ?? 'erreur inconnue'}` };
      const focused = (r as Record<string, unknown>)['focused'];
      const focusTag = focused === true ? ' et mis au premier plan' : '';
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:app_launch', message: `🚀 *${app}* lancé${focusTag}.` };
    }
    ...
    case 'app_close': {
      ...
      if (!r.ok) return { ok: false, toolUsed: 'nexus:window_close', message: `❌ Fermeture ${app} échouée : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:window_close', message: `🔴 *${app}* fermé.` };
    }

    case 'focus_app': {
      const app = args['app'] as string;
      const r = await nexusFocusApp(app, 10_000);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:focus_app', message: `❌ Focus ${app} échoué : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:focus_app', message: `🎯 *${app}* mis au premier plan.` };
    }

    case 'url_open': {
      const url = args['url'] as string;
      const app = (args['app'] as string | undefined) ?? url;
      const r = await nexusOpenUrl(url, 10_000);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:open_url', message: `❌ Ouverture ${app} échouée \\(exit ${r.exit_code}\\)` };
    ...
    case 'file_list': {
      ...
      if (!r.ok) return { ok: false, toolUsed: 'nexus:file_list', message: `❌ Liste fichiers échouée : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:file_list', message: fmtFileList(r as Record<string, unknown>) };
    }

    case 'file_search': {
      const query = args['query'] as string;
      const r = await nexusFileSearch(query);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:file_search', message: `❌ Recherche échouée : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:file_search', message: fmtFileSearch(r as Record<string, unknown>) };
    }

    case 'file_read': {
      const path = args['path'] as string;
      const r = await nexusFileRead(path);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:file_read', message: `❌ Lecture échouée : ${r.error ?? 'erreur inconnue'}` };
      const content = (r['content'] as string | undefined)?.slice(0, 3000) ?? '(vide)';
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:file_read', message: `📄 *Contenu* :\n\`\`\`\n${content}\n\`\`\`` };
    }

    case 'file_send': {
      const path = args['path'] as string;
      const r = await nexusFileSend(path);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:file_send', message: `❌ Envoi échoué : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:file_send', message: `📤 Fichier envoyé sur Telegram.` };
    }

    case 'file_open': {
      const path = args['path'] as string;
      const r = await nexusFileOpen(path);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:file_open', message: `❌ Ouverture échouée : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:file_open', message: `📂 Fichier ouvert.` };
    }

    case 'window_list': {
      const r = await nexusWindowList();
      if (!r.ok) return { ok: false, toolUsed: 'nexus:window_list', message: `❌ Liste fenêtres échouée : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:window_list', message: fmtWindowList(r as Record<string, unknown>) };
    }

    case 'window_focus': {
      const title = args['title'] as string;
      const r = await nexusWindowFocus(title);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:window_focus', message: `❌ Focus échoué : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:window_focus', message: `🪟 Fenêtre "${title}" mise en avant.` };
    }

    case 'window_close': {
      const title = args['title'] as string;
      const r = await nexusWindowClose(title);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:window_close', message: `❌ Fermeture échouée : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:window_close', message: `🪟 Fenêtre "${title}" fermée.` };
    }

    case 'window_screenshot': {
      const r = await nexusWindowScreenshot();
      if (!r.ok) return { ok: false, toolUsed: 'nexus:window_screenshot', message: `❌ Screenshot fenêtre échoué : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:window_screenshot', message: `📸 Screenshot fenêtre envoyé sur Telegram.` };
    }

    case 'process_list': {
      const sort = (args['sort'] as 'ram' | 'cpu') ?? 'ram';
      const r = await nexusProcessList(20, sort);
      if (!r.ok) return { ok: false, toolUsed: 'nexus:process_list', message: `❌ Liste processus échouée : ${r.error ?? 'erreur inconnue'}` };
      return { ok: true, jobId: r['job_id'] as string | undefined, toolUsed: 'nexus:process_list', message: fmtProcessList(r as Record<string, unknown>) };
    }
    ...
    case 'process_kill': {
      ...
      if (!r.ok) return { ok: false, toolUsed: 'nexus:process_kill', message: `❌ Kill échoué : ${r.error ?? 'erreur inconnue'}` };
...
export async function routeNexusMessage(text: string): Promise<NlRouterResult> {
  ...
  for (const cmd of cmds) {
    ...
    // ── Log: INTENT_DETECTED
    logs.push(
      `[INTENT_DETECTED] cmd="${cmd.slice(0, 80)}" intent=${intent.type} ` +
    ...
    if (intent.type === 'unknown' && intent.sysPriority) {
      ...
      messages.push(
        `⚠️ *Commande système non reconnue*\n` +
        `Commande : _${cmd}_\n` +
    ...
    logs.push(
      `[ACTION_SELECTED] intent=${intent.type} ` +
    ...
    try {
      ...
      logs.push(
        `[TOOL_USED] tool=${result.toolUsed ?? 'unknown'} intent=${intent.type} ` +
...
function _toolName(intent: DetectedIntent): string {
