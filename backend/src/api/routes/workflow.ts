...
async function runNexusProof(jobId: string): Promise<void> {
  ...
  async function step(
    name: string,
    fn: () => Promise<{ ok: boolean; exit_code?: number; stdout?: string; stderr?: string; detail?: string; blocked?: boolean; cmd_job_id?: string }>,
  ): Promise<void> {
    const t0 = Date.now();
    try {
      const r = await fn();
  ...
  let sysinfoData: Record<string, unknown> = {};
  await step('sysinfo (Python + OS)', async () => {
    const r = await nexusSysinfo(10_000);
    if (r.ok) sysinfoData = r as unknown as Record<string, unknown>;
    return {
      ok: r.ok,
      detail: r.ok
  ...
  await step('git status', async () => {
    const r = await nexusRunCommand('git status', repoDir, 15_000);
    return { ok: r.ok, exit_code: r.exit_code, stdout: r.stdout, stderr: r.stderr, cmd_job_id: r.jobId };
  });
  ...
  await step('py --version (Windows Launcher)', async () => {
    const r = await nexusRunCommand('py --version', undefined, 10_000);
    if (r.ok) return { ok: true, exit_code: 0, stdout: r.stdout, cmd_job_id: r.jobId };
    // fallback: try python3
    const r2 = await nexusRunCommand('python3 --version', undefined, 10_000);
  ...
  await step('verify file on disk', async () => {
    const r = await nexusRunCommand(
      `powershell -Command "Get-Item '${desktopFile}' | Select-Object FullName,Length,LastWriteTime | ConvertTo-Json"`,
      undefined, 10_000,
    );
    return { ok: r.ok, exit_code: r.exit_code, stdout: r.stdout, stderr: r.stderr, cmd_job_id: r.jobId };
  });
  ...
  await step('screenshot → Telegram', async () => {
    const r = await nexusScreenshot(
      `📸 NEXUS Proof — ${new Date().toLocaleString('fr-FR')} — jobId: ${jobId}`,
      30_000,
    );
    return {
      ok:     r.ok,
  ...
  await step('npx tsc --noEmit', async () => {
    const r = await nexusRunCommand('npx tsc --noEmit --skipLibCheck', backendDir, 90_000);
    return {
      ok:         r.ok,
  ...
  job.hostname     = (sysinfoData['hostname'] as string | undefined) ?? nexusStatus.telemetry.lastHostname ?? 'unknown';
