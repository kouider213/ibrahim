...
${financeReport.bookings.map((b: any) => `- ${b.client_name} | ${b.car_name} | ${b.nb_days}j | client: ${b.client_price_per_day ?? '?'}€/j — Houari: ${b.owner_price_per_day ?? '?'}€/j | Total client: ${b.final_price ?? '?'}€ | Bénéfice Kouider: ${b.kouider_profit != null ? `${b.kouider_profit}€` : '❓ (owner_ppd manquant)'}`).join('\n')}`
  ...
  const actorPersonaText = isHouari
    ? `\n\nIDENTITÉ DZARYX — HOUARI:\nTu es le Dzaryx personnel de Houari (associé Fik Conciergerie Oran).\nLANGUE PAR DÉFAUT: Darija oranaise algérienne. Réponds TOUJOURS en darija sauf si Houari écrit clairement en français ou anglais.\nLOCALISATION: Houari est physiquement à ORAN — il peut gérer les opérations terrain directement.\nFOCUS: priorité aux infos et réservations de Houari (rented_by="Houari"). Tu connais aussi les infos de Kouider.\nRENTED_BY PAR DÉFAUT: quand Houari crée une réservation → rented_by="Houari" automatiquement.\n${vipList}`
