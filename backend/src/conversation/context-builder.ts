...
• Une courte phrase peut précéder le bloc. Mets le bloc \`\`\`chart seulement si un visuel est pertinent.`
  ...
  if (actor.ownerKey === 'houari') {
    // Houari = MIROIR de langue : français→français, espagnol→espagnol,
    // arabe/darija→darija 100% ORANAIS (jamais marocain).
    if (['ar', 'darija', 'fr+darija'].includes(langDetection.lang)) {
      langDetection = {
        lang:       'darija',
        label:      'Darija 100% oranaise (Houari)',
        systemHint: 'LANGUE: Houari a parlé en DARIJA → réponds ENTIÈREMENT en DARIJA ORANAISE. ⛔ RÈGLE ABSOLUE: TOUTE la réponse en darija (structure de phrase darija) — JAMAIS des phrases en français. Tu gardes SEULEMENT les mots français qu\'un Oranais dit vraiment (cliene, réservation, acompte, dispo, tomobil/voiture) — c\'est le vrai oranais. Mais le reste = darija: wesh, rak, khoya, sahbi, labas, bzaf, mliih, koulech, kifach, chhal/gdach, fin, 3lach, derk, yezi, drahem, nchallah, ghaya, ndir, kayen, makanch. ⛔ JAMAIS "dima" → "toujours". ⛔ INTERDIT marocain: ghadi, daba, khassek, zwin, mzyan, dyal, wakha, bghit. ⚠️ MÊME quand tu rapportes des DONNÉES (réservations, news, chiffres, résultats de recherche en français) → reformule TOUT en darija oranaise (ex: "3andek 2 réservations" pas "Voilà les 2 réservations"). 🔴 MAXIMUM EN ARABE: écris de préférence en LETTRES ARABES et traduis les libellés/catégories en arabe (Citadines→سيارات صغيرة, Berlines→برلين, acompte→التسبيق, reste→الباقي, payé→خلّص, terminées→مكملين, disponible→ديسبو/متاح). Garde en latin/français SEULEMENT: les noms propres (modèles: Clio, Jumpy, Fiat 500), les montants (1200€) et les rares mots oranais (cliene). Le reste = arabe. Parle l\'oranais PARFAITEMENT, naturel, comme un vrai habitant d\'Oran. Ton chaleureux, respectueux (patron PDG).',
      };
      ...
      langDetection = { lang: 'en', label: 'Anglais (Houari)', systemHint: 'LANGUE: réponds en anglais.' };
    } else {
      // français (ou inconnu/court) → français
    ...
    if (['ar', 'darija', 'fr+darija'].includes(langDetection.lang)) {
      langDetection = {
        ...
        systemHint: 'LANGUE: Kouider a parlé en DARIJA → COMPRENDS l\'algérien et réponds ENTIÈREMENT en DARIJA ORANAISE. ⛔ RÈGLE ABSOLUE: TOUTE la réponse en darija (structure de phrase darija) — JAMAIS des phrases en français. Tu gardes SEULEMENT les mots français qu\'un Oranais dit vraiment (cliene, réservation, acompte, dispo, tomobil/voiture). Le reste = darija: wesh, rak, khoya, sahbi, labas, bzaf, mliih, koulech, kifach, chhal/gdach, fin, 3lach, derk, yezi, drahem, nchallah, ndir, kayen, makanch. ⛔ JAMAIS "dima" → "toujours". ⛔ INTERDIT marocain: ghadi, daba, khassek, zwin, mzyan, dyal, wakha, bghit. ⚠️ MÊME quand tu rapportes des DONNÉES (réservations, news, chiffres, résultats de recherche en français) → reformule TOUT en darija oranaise (ex: "3andek 2 réservations jayin" pas "Voilà les 2 réservations qui viennent"). 🔴 MAXIMUM EN ARABE: écris de préférence en LETTRES ARABES et traduis les libellés/catégories en arabe (Citadines→سيارات صغيرة, Berlines→برلين, acompte→التسبيق, reste→الباقي, payé→خلّص, terminées→مكملين, disponible→متاح). Garde en latin/français SEULEMENT: noms propres (modèles: Clio, Jumpy, Fiat 500), montants (1200€) et les rares mots oranais (cliene). Le reste = arabe. Parle l\'oranais PARFAITEMENT et naturel. ⚠️ Tu PARLES À Kouider directement (2e personne) — PAS une conversation rapportée entre tiers.',
      };
    } else if (langDetection.lang === 'es') {
