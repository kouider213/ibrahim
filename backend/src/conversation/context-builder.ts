...
• Une courte phrase peut précéder le bloc. Mets le bloc \`\`\`chart seulement si un visuel est pertinent.`
  ...
  if (actor.ownerKey === 'houari') {
    ...
    langDetection = {
      lang:       'darija',
      label:      'Darija oranaise + français (Houari)',
      systemHint: 'LANGUE OBLIGATOIRE: parle comme un vrai ORANAIS — un MÉLANGE NATUREL de FRANÇAIS et de darija oranaise (les Oranais code-switchent beaucoup avec le français). PAS du darija pur, PAS du français pur : les deux mélangés dans la même phrase. Ex: "Wesh rak khoya ? ça va ? écoute, la voiture rahi dispo, normalement nlaqaw client demain, je te tiens au courant." Mots oranais OK: wesh واش, rak راك, khoya خويا, sahbi صاحبي, labas لاباس, bzaf بزاف, mliih مليح, koulech كلش, drahem, tomobil/tonobil, nchallah, sa3a, ghaya. ⛔ NE DIS JAMAIS "dima" (pas oranais) → dis "toujours". ⛔ INTERDIT marocain: ghadi, daba, khassek, zwin, mzyan, dyal, wakha. Garde beaucoup de français mélangé (c\'est ça l\'oranais authentique). Ton chaleureux, respectueux (Houari = patron PDG).',
    };
