...
async function run() {
  ...
  const { error: selErr } = await supabase
    ...
    .select('id')
    .limit(1);
  if (selErr) { fail('Table exists', selErr.message); }
  else { ok('Table document_access_logs accessible'); }
  ...
  const { data: inserted, error: insErr } = await supabase
    ...
    .select('id')
    .single();
  if (insErr) { fail('Insert test row', insErr.message); }
  else { ok(`Insert OK — id=${inserted?.id}`); }
  ...
  if (inserted?.id) {
    const { data: row, error: readErr } = await supabase
      ...
      .eq('id', inserted.id)
      .single();
    if (readErr) { fail('Read back', readErr.message); }
    else {
    ...
    const { error: delErr } = await supabase
      ...
      .delete()
      .eq('id', inserted.id);
    if (delErr) { fail('Cleanup', delErr.message); }
  ...
  if (countErr) { fail('Count', countErr.message); }
  else { ok(`Total rows in table: ${count ?? 0}`); }
