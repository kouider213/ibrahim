...
export async function savePropertyBeforeState(
  ...
  try {
    const [analysis, photoUrl, resolvedProp] = await Promise.all([
      analyzeInspectionPhoto(base64, mime, 'property', 'before'),
      uploadInspectionPhoto(base64, mime, 'inspections/properties'),
    ...
    const message = analysis.damageDetected
      ? `🏠 *État AVANT — ${propertyName} (${clientName})*\n\n${analysis.description}\n\n⚠️ *Défauts existants :*\n${formatDamageLines(analysis.damageBoxes)}\n\n_Dossier ${data.id.slice(-8)}_`
      : `🏠 *État AVANT — ${propertyName} (${clientName})*\n\n${analysis.description}\n\n✅ *Aucun défaut constaté.*\n\n_Dossier ${data.id.slice(-8)}_`;

    return { success: true, message, stateId: data.id, photoUrl, bookingId: resolvedProp, analysis };
  } catch (e) {
...
export async function savePropertyAfterState(
  ...
  try {
    ...
    const [analysis, photoUrl] = await Promise.all([
      analyzeInspectionPhoto(base64, mime, 'property', 'after', beforeState.ai_description ?? ''),
      uploadInspectionPhoto(base64, mime, 'inspections/properties'),
    ]);
    ...
    const message = analysis.damageDetected
      ? `🔍 *Rapport sortie — ${propertyName} (${clientName})*\n\n📅 État d'entrée : ${dateAvant}\n\n🆕 *Nouveaux défauts :*\n${formatDamageLines(newDmg)}\n\n📝 ${analysis.comparisonReport ?? analysis.description}\n\n_Dossier ${data.id.slice(-8)}_`
      : `✅ *Sortie OK — ${propertyName} (${clientName})*\n\n📅 État d'entrée : ${dateAvant}\n\nAucun nouveau défaut par rapport à l'entrée.\n\n${analysis.description}\n\n_Dossier ${data.id.slice(-8)}_`;

    return { success: true, message, stateId: data.id, photoUrl, bookingId: beforeState.property_id ?? null, analysis };
  } catch (e) {
