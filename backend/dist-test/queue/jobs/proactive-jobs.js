...
exports.jobMorningBriefing = jobMorningBriefing;
exports.jobEndRentalReminder = jobEndRentalReminder;
exports.jobIdleVehicleAlert = jobIdleVehicleAlert;
exports.jobTikTokSuggestion = jobTikTokSuggestion;
exports.jobWednesdayContent = jobWednesdayContent;
exports.jobFridayContent = jobFridayContent;
exports.jobUnpaidReminder = jobUnpaidReminder;
exports.jobLateReturnAlert = jobLateReturnAlert;
exports.jobCheckAnomalies = jobCheckAnomalies;
exports.jobWeeklyReport = jobWeeklyReport;
exports.jobPatternDetection = jobPatternDetection;
exports.jobWhatsAppBookingConfirmations = jobWhatsAppBookingConfirmations;
exports.jobWhatsApp24hReminders = jobWhatsApp24hReminders;
exports.jobWhatsAppReturnReminders = jobWhatsAppReturnReminders;
exports.jobCompetitorWatch = jobCompetitorWatch;
exports.jobBIDaily = jobBIDaily;
exports.jobBIReminders = jobBIReminders;
exports.jobAnthropicWatch = jobAnthropicWatch;
...
async function tg(text) {
    await (0, telegram_js_1.sendMessage)(ownerChatId(), text);
}
// ── 0. Réveil matinal 7h30 ────────────────────────────────────
async function jobMorningBriefing(_job) {
    const today = new Date().toISOString().slice(0, 10);
    const tomorrow = new Date();
    tomorrow.setDate(tomorrow.getDate() + 1);
    const tomorrowStr = tomorrow.toISOString().slice(0, 10);
    const [activeBookings, returningToday, returningTomorrow, upcomingBookings, weather, calendarEvents, financeThisMonth,] = await Promise.all([
        supabase_js_1.supabase.from('bookings').select('*, cars(name)')
        ...
        (0, web_search_js_1.getOranWeather)().catch(() => null),
        (0, google_calendar_js_1.listUpcomingEvents)(5).catch(() => []),
        (0, finance_js_1.getFinancialReport)(new Date().getFullYear(), new Date().getMonth() + 1).catch(() => null),
    ]);
    const now = new Date();
    ...
    // Locations en cours
    const actives = (activeBookings.data ?? []);
    if (actives.length > 0) {
    ...
    // Retours aujourd'hui
    const retToday = (returningToday.data ?? []);
    if (retToday.length > 0) {
        lines.push(`🔑 *Retours AUJOURD'HUI:*`);
        for (const b of retToday)
            lines.push(`  • ${b.client_name} — ${b.cars?.name ?? '?'}`);
        lines.push(``);
    }
    // Retours demain
    const retTomorrow = (returningTomorrow.data ?? []);
    if (retTomorrow.length > 0) {
        lines.push(`📅 *Retours DEMAIN:*`);
        for (const b of retTomorrow)
            lines.push(`  • ${b.client_name} — ${b.cars?.name ?? '?'}`);
        lines.push(``);
    }
    ...
    if (upcoming.length > 0) {
        ...
        for (const b of upcoming)
            lines.push(`  • ${b.client_name} — ${b.cars?.name ?? '?'} le ${b.start_date}`);
        lines.push(``);
    }
    // Agenda Google
...
function getDailyTip(dayOfWeek, activeCount, upcomingCount) {
    ...
    if (dayOfWeek === 1)
        return 'Début de semaine — vérifie les docs de tous les clients en cours.';
    if (dayOfWeek === 5)
        return 'Vendredi — vérifie les retours prévus ce week-end.';
    if (dayOfWeek === 0)
        return 'Dimanche — bon moment pour planifier la semaine avec Dzaryx.';
    return '';
}
// ── 1. Rappel fin réservation ─────────────────────────────────
async function jobEndRentalReminder(_job) {
    const tomorrow = new Date();
    tomorrow.setDate(tomorrow.getDate() + 1);
    const tomorrowStr = tomorrow.toISOString().split('T')[0];
    const { data: bookings } = await supabase_js_1.supabase
    ...
    for (const b of bookings) {
        ...
        await tg(msg);
        await (0, pushover_js_1.notifyOwner)('🚗 Fin de réservation demain', `${b.client_name} — ${carName}`, false);
    }
    console.log(`[job:end-rental] ${bookings.length} reminder(s) sent`);
}
// ── 2. Véhicule sans réservation 7j ──────────────────────────
async function jobIdleVehicleAlert(_job) {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - 7);
    const cutoffStr = cutoff.toISOString().split('T')[0];
    const { data: cars } = await supabase_js_1.supabase.from('cars').select('id, name, base_price').eq('available', true);
    if (!cars?.length)
        return;
    const idleCars = [];
    ...
    await (0, pushover_js_1.notifyOwner)(`⚠️ ${idleCars.length} véhicule(s) idle`, list, false);
    console.log(`[job:idle-vehicle] ${idleCars.length} idle`);
}
// ── 3. Marketing TikTok hebdomadaire (IA complète) ────────────
async function jobTikTokSuggestion(_job) {
    ...
    const report = await (0, market_research_js_1.runTikTokMarketResearch)(cars).catch(err => {
    ...
    const targetCar = cars.find(c => bestIdea.car_suggestion &&
        c.name.toLowerCase().includes(bestIdea.car_suggestion.toLowerCase())) ?? cars[0];
    ...
    const videoResult = await (0, video_creator_js_1.createMarketingVideo)(targetCar, bestIdea).catch(err => {
    ...
    await (0, telegram_js_1.sendVideoBuffer)(ownerChatId(), videoResult.buffer, approvalCaption).catch(async () => {
        await tg([
            approvalCaption,
            ``,
    ...
    await (0, pushover_js_1.notifyOwner)('📱 Vidéo TikTok prête', `${bestIdea.title} — réponds Oke pour publier`, false);
    console.log('[job:tiktok] Weekly marketing job complete');
}
// ── Phase 5: Mercredi lifestyle + Vendredi prix choc ─────────
async function createWeeklyVideo(style, label) {
    const { data: carsRaw } = await supabase_js_1.supabase.from('cars').select('*').eq('available', true);
    const cars = (carsRaw ?? []).filter(c => c.image_url);
    if (cars.length === 0) {
    ...
    const { executeCreateMarketingVideo } = await Promise.resolve().then(() => __importStar(require('../../marketing/create-marketing-video.js')));
    let result = null;
    try {
        result = await executeCreateMarketingVideo({ car_name: car.name, style }, ownerChatId());
    }
    catch (err) {
...
// ════════════════════════════════════════════════════════════════
async function jobUnpaidReminder(_job) {
    ...
    const { data: activeUnpaid } = await supabase_js_1.supabase
        .from('bookings')
    ...
    const { data: pendingNoDeposit } = await supabase_js_1.supabase
        .from('bookings')
        .select('id, client_name, client_phone, final_price, paid_amount, payment_status, created_at, start_date, end_date, cars(name)')
    ...
    for (const booking of (activeUnpaid ?? [])) {
        ...
        if (!attempt1Log) {
            ...
            await tg(tgMsg);
            await supabase_js_1.supabase.from('relance_logs').insert({
            ...
            if (hoursSince2 >= 24) {
                ...
                await supabase_js_1.supabase.from('relance_logs').update({ sent_at: now.toISOString(), status: 'urgent' })
    ...
    for (const booking of (pendingNoDeposit ?? [])) {
        const clientName = booking.client_name;
        const clientPhone = booking.client_phone;
        const carName = booking.cars?.name ?? 'Véhicule';
        const total = Number(booking.final_price ?? 0);
        ...
        const { data: logs } = await supabase_js_1.supabase
            .from('relance_logs').select('attempt').eq('booking_id', booking.id);
        if ((logs ?? []).length > 0)
        ...
        await supabase_js_1.supabase.from('relance_logs').insert({
            booking_id: booking.id, client_name: clientName, client_phone: clientPhone,
...
function generateSoldeMessage(clientName, remaining, carName, attempt, daysWithCar) {
    if (attempt === 1) {
...
function buildTelegramSolde(clientName, clientPhone, carName, remaining, total, paid, attempt, daysWithCar, startDate) {
...
async function jobLateReturnAlert(_job) {
    const today = new Date().toISOString().slice(0, 10);
    const { data: overdue } = await supabase_js_1.supabase
        .from('bookings')
    ...
    for (const b of overdue) {
        ...
        await (0, pushover_js_1.notifyOwner)(`🚨 Retard ${daysLate}j — ${b.client_name}`, `${carName} — devait rendre le ${b.end_date}`, true);
    }
    console.log(`[job:late-return] ${overdue.length} véhicule(s) en retard détecté(s)`);
}
// ── 4b. Détection anomalies financières ──────────────────────
async function jobCheckAnomalies(_job) {
    try {
        const { checkAnomalies } = await Promise.resolve().then(() => __importStar(require('../../integrations/phase5-finance.js')));
        const result = await checkAnomalies();
        if (result && !result.includes('Aucune anomalie')) {
            await tg(`⚠️ *Anomalies financières détectées:*\n${result}`);
            await (0, pushover_js_1.notifyOwner)('⚠️ Anomalie financière', result.slice(0, 200), true);
        }
...
async function jobWeeklyReport(_job) {
    ...
    // Finance du mois
    const finance = await (0, finance_js_1.getFinancialReport)(new Date().getFullYear(), new Date().getMonth() + 1).catch(() => null);
    const report = [
    ...
    await tg(report);
    await (0, pushover_js_1.notifyOwner)('📊 Rapport hebdomadaire', report, false);
    console.log('[job:weekly-report] sent');
}
// ── 6. Détection patterns (lundi avec rapport hebdo) ─────────
async function jobPatternDetection(_job) {
    ...
    const lines = ['📈 *Patterns détectés (3 derniers mois):*', ''];
...
async function jobWhatsAppBookingConfirmations(_job) {
    if (!env_js_1.env.TWILIO_ACCOUNT_SID || !env_js_1.env.TWILIO_AUTH_TOKEN || !env_js_1.env.TWILIO_WHATSAPP_FROM) {
    ...
    for (const b of bookings) {
        ...
        const lang = (0, whatsapp_js_1.detectLanguage)(''); // default fr, phone has no text to detect
        ...
        const ok = await (0, whatsapp_js_1.sendWhatsApp)(phone, msg);
        if (ok) {
            await supabase_js_1.supabase.from('bookings').update({ whatsapp_sent: true }).eq('id', b.id);
            console.log(`[job:wa-confirm] ✅ Sent to ${phone}`);
        }
    }
}
// Rappel 24h avant prise en charge
async function jobWhatsApp24hReminders(_job) {
    if (!env_js_1.env.TWILIO_ACCOUNT_SID || !env_js_1.env.TWILIO_AUTH_TOKEN || !env_js_1.env.TWILIO_WHATSAPP_FROM) {
...
async function jobCompetitorWatch(_job) {
    console.log('[job:competitor-watch] Démarrage veille concurrence...');
    try {
        const { multiProviderWebSearch } = await Promise.resolve().then(() => __importStar(require('../../integrations/web-search-provider.js')));
        ...
        const searchResults = await Promise.allSettled(SEARCH_QUERIES.map(q => multiProviderWebSearch(q).then(r => ({ q, text: r.text, source: r.source, chars: r.results_count }))));
        const results = searchResults
            .map((r, i) => {
            if (r.status === 'rejected')
                return `[${SEARCH_QUERIES[i]}]\n❌ Erreur recherche`;
        ...
        const { formatPricingTable } = await Promise.resolve().then(() => __importStar(require('../../config/pricing.js')));
        const pricing = formatPricingTable();
        const { data: carsRaw } = await supabase_js_1.supabase.from('cars').select('name, resale_price').eq('available', true);
        const availableNames = (carsRaw ?? []).map((c) => `${c.name} (${c.resale_price}€/j)`).join(', ');
...
async function jobBIDaily(_job) {
    console.log('[job:bi-daily] Démarrage rapport BI quotidien...');
    try {
        const { runBIEngine } = await Promise.resolve().then(() => __importStar(require('../../bi/bi-engine.js')));
...
async function jobBIReminders(_job) {
    try {
        const { getSmartReminders } = await Promise.resolve().then(() => __importStar(require('../../bi/smart-reminders.js')));
        ...
        for (const r of highPri.slice(0, 5)) {
            ...
            await (0, telegram_js_1.sendMessage)(chatId, `${emoji} *${r.message}*\n💡 ${r.action}`);
        }
...
async function jobAnthropicWatch(_job) {
    try {
        // Fetch release notes + SDK changelog via Jina.ai
        const [releaseNotes, sdkChangelog] = await Promise.all([
            axios_1.default.get('https://r.jina.ai/https://docs.anthropic.com/en/release-notes/overview', {
                headers: { 'Accept': 'text/plain', 'X-Retain-Images': 'none' },
                timeout: 20_000,
            }).then((r) => (typeof r.data === 'string' ? r.data : JSON.stringify(r.data)).slice(0, 3000)),
            axios_1.default.get('https://r.jina.ai/https://github.com/anthropics/anthropic-sdk-node/blob/main/CHANGELOG.md', {
                headers: { 'Accept': 'text/plain', 'X-Retain-Images': 'none' },
                timeout: 20_000,
            }).then((r) => (typeof r.data === 'string' ? r.data : JSON.stringify(r.data)).slice(0, 2000)),
        ]);
        const analysis = await (0, claude_api_js_1.chat)([{
                role: 'user',
                content: `Tu es Dzaryx, assistant IA de Fik Conciergerie Oran.
...
//# sourceMappingURL=proactive-jobs.js.map