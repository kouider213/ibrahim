...
const google_calendar_js_1 = require("./google-calendar.js");
const finance_js_1 = require("./finance.js");
const media_executor_js_1 = require("./media-executor.js");
const github_js_1 = require("./github.js");
const claude_api_js_1 = require("./claude-api.js");
...
const market_research_js_1 = require("../marketing/market-research.js");
const video_creator_js_1 = require("../marketing/video-creator.js");
const approval_store_js_1 = require("../marketing/approval-store.js");
const video_session_store_js_1 = require("../marketing/video-session-store.js");
const create_marketing_video_js_1 = require("../marketing/create-marketing-video.js");
const scene_assembler_js_1 = require("../marketing/scene-assembler.js");
const video_project_store_js_1 = require("../marketing/video-project-store.js");
const PROJ_W = 1080;
const PROJ_H = 1920;
...
const supabase_js_2 = require("./supabase.js");
const phase5_finance_js_1 = require("./phase5-finance.js");
const feedback_system_js_1 = require("./feedback-system.js");
const improvement_report_js_1 = require("./improvement-report.js");
const form_data_1 = __importDefault(require("form-data"));
const whatsapp_js_1 = require("./whatsapp.js");
const telegram_js_2 = require("./telegram.js");
const generate_voucher_js_1 = require("./generate-voucher.js");
...
const memory_engine_js_1 = require("../orchestrator/memory-engine.js");
const axios_1 = __importDefault(require("axios"));
const crypto_1 = __importDefault(require("crypto"));
const code_agent_js_1 = require("../agents/code-agent.js");
const image_to_image_js_1 = require("./image-to-image.js");
const web_search_provider_js_1 = require("./web-search-provider.js");
...
const FAIL_PHRASE_PATTERNS = [
    /\bintrouvable\b/i, // "Réservation introuvable", "Client introuvable"
    /\bnot\s+found\b/i, // English not-found
];
function isToolFailureResult(result, toolName) {
    if (result.startsWith(`Outil inconnu: ${toolName}`))
        return true;
    if (FAIL_START_PATTERNS.some(p => p.test(result)))
        return true;
    if (FAIL_PHRASE_PATTERNS.some(p => p.test(result)))
        return true;
    return false;
}
...
async function executeTool(name, input, sessionId) {
    ...
    (0, action_engine_js_1.recordToolExecution)({
        sessionId: sid,
        toolName: name,
        args: input,
        result,
...
async function _dispatch(name, input, sessionId) {
    try {
        switch (name) {
            ...
            case 'railway_wait_deploy': return await (0, railway_js_1.waitForDeploy)(Number(input['timeout_seconds'] ?? 180) * 1000);
            ...
            // ─── PHASE 5 ───
            case 'get_payment_status': return await (0, phase5_finance_js_1.getPaymentStatus)(input['booking_id']);
            case 'record_payment': return await (0, phase5_finance_js_1.recordPayment)(input['booking_id'], Number(input['amount']), input['type'] ?? 'partiel', input['note']);
            case 'get_revenue_report': return await (0, phase5_finance_js_1.getCAReport)(input['year'] ? Number(input['year']) : new Date().getFullYear(), input['month'] ? Number(input['month']) : undefined, input['week'] ? Number(input['week']) : undefined);
            case 'get_unpaid_bookings': return await (0, phase5_finance_js_1.getUnpaidBookings)();
            case 'generate_receipt': return await (0, phase5_finance_js_1.generateReceipt)(input['booking_id']);
            case 'get_finance_dashboard': return await (0, phase5_finance_js_1.getFinancialDashboard)();
            case 'check_anomalies': return await (0, phase5_finance_js_1.checkAnomalies)();
            // ─── PHASE 13 ───
            ...
            case 'create_video_preview': return await (0, media_executor_js_1.executeMediaTool)(name, input);
            // ─── MARKETING TIKTOK ───
            ...
            case 'transform_image': return await (0, image_to_image_js_1.executeImageToImage)(input, sessionId);
...
async function listBookings(input) {
    ...
    if (error) {
        const { data: fallback, error: err2 } = await supabase_js_1.supabase
            .from('bookings')
            .select('id, client_name, client_phone, start_date, end_date, final_price, status')
            .order('start_date', { ascending: false })
            .limit(Number(input['limit'] ?? 20));
        if (err2)
            return `Erreur: ${err2.message}`;
        if (!fallback?.length)
            return 'Aucune réservation trouvée.';
        return `${fallback.length} réservation(s):\n${fallback.map(b => `- [${b.id}] ${b.client_name} | ${b.start_date} → ${b.end_date} | ${b.final_price}€ | ${b.status}`).join('\n')}`;
    }
    if (!data?.length)
        return 'Aucune réservation trouvée.';
...
async function updateBooking(input) {
    ...
    if (input['client_phone'])
        fields['client_phone'] = input['client_phone'];
    if (input['client_age'])
        fields['client_age'] = input['client_age'];
    if (input['start_date'])
        fields['start_date'] = input['start_date'];
    if (input['end_date'])
        fields['end_date'] = input['end_date'];
    if (input['final_price'] !== undefined)
        fields['final_price'] = input['final_price'];
    if (input['status'])
        fields['status'] = input['status'];
    if (input['payment_status']) {
        // Automotolux values: UNPAID | PARTIAL | PAID (uppercase)
        const psMap = { pending: 'UNPAID', unpaid: 'UNPAID', partial: 'PARTIAL', paid: 'PAID', PENDING: 'UNPAID', UNPAID: 'UNPAID', PARTIAL: 'PARTIAL', PAID: 'PAID' };
        fields['payment_status'] = psMap[input['payment_status']] ?? input['payment_status'].toUpperCase();
    }
    if (input['paid_amount'] !== undefined)
        fields['paid_amount'] = Number(input['paid_amount']);
    if (input['rented_by'])
        fields['rented_by'] = input['rented_by'];
    if (input['notes'])
        fields['notes'] = input['notes'];
    const { error } = await supabase_js_1.supabase.from('bookings').update(fields).eq('id', id);
    if (error)
        return `Erreur mise à jour: ${error.message}`;
    return `✅ Réservation ${id} mise à jour: ${JSON.stringify(fields)}`;
}
async function createBooking(input) {
    if (!input['client_name'])
        return '❌ client_name manquant';
    if (!input['start_date'])
        return '❌ start_date manquant (format YYYY-MM-DD)';
    if (!input['end_date'])
        return '❌ end_date manquant (format YYYY-MM-DD)';
    if (input['start_date'] > input['end_date'])
        return '❌ start_date doit être avant end_date';
    // Resolve car_id from car_name if UUID not provided
    let carId = input['car_id'];
    if (!carId && input['car_name']) {
        const { data: carMatch } = await supabase_js_1.supabase
            .from('cars')
            .select('id, name')
            ...
            .single();
        if (!carMatch)
            return `❌ Voiture "${input['car_name']}" introuvable dans la flotte. Vérifie le nom exact.`;
        carId = carMatch.id;
    }
    if (!carId)
        return '❌ car_id ou car_name manquant — spécifie la voiture';
    const VALID_STATUSES = ['CONFIRMED', 'PENDING', 'ACTIVE', 'COMPLETED', 'REJECTED'];
    const status = input['status'] ?? 'CONFIRMED';
    if (!VALID_STATUSES.includes(status))
        return `❌ status invalide: ${status}. Valeurs: ${VALID_STATUSES.join(', ')}`;
    // Anti-doublon: vérifie disponibilité avant insertion
    const isAvailable = await (0, supabase_js_2.checkCarAvailability)(carId, input['start_date'], input['end_date']);
    if (!isAvailable) {
    ...
    const nb_days = Math.max(1, Math.ceil((new Date(input['end_date']).getTime() - new Date(input['start_date']).getTime()) / 86_400_000));
    // Vérification prix Houari — refuse si prix client < coût Houari
    try {
        const { data: carRow } = await supabase_js_1.supabase.from('cars').select('name').eq('id', carId).single();
        const carNameForCheck = carRow?.name;
        if (carNameForCheck) {
            const { data: pricingRow } = await supabase_js_1.supabase.from('pricing').select('houari_price').ilike('vehicle_name', `%${carNameForCheck}%`).limit(1).single();
            const houariDay = pricingRow ? Number(pricingRow.houari_price) : null;
            if (houariDay !== null) {
    ...
    let calendarNote = '';
    try {
        const { data: car } = await supabase_js_1.supabase.from('cars').select('name').eq('id', carId).single();
        const carName = car?.name ?? 'Véhicule';
        const eventId = await (0, google_calendar_js_1.createCalendarEvent)(booking.id, input['client_name'], carName, input['start_date'], input['end_date'], input['notes']);
        calendarNote = eventId ? ' | 📅 Ajouté Google Agenda' : ' | ⚠️ Google Agenda non synchro';
    }
    catch {
        calendarNote = ' | ⚠️ Google Agenda non synchro';
    }
...
async function cancelBooking(input) {
    const { error } = await supabase_js_1.supabase
        ...
        .eq('id', input['id']);
    if (error)
        return `Erreur annulation: ${error.message}`;
    return `✅ Réservation ${input['id']} annulée (REJECTED)`;
}
async function deleteBooking(input) {
    const id = input['id'];
    const { data: booking } = await supabase_js_1.supabase.from('bookings').select('status').eq('id', id).single();
    if (!booking)
        return `❌ Réservation ${id} introuvable`;
    ...
    let calendarNote = '';
    try {
        const { data: calEvent } = await supabase_js_1.supabase
            .from('calendar_events')
            .select('google_event_id')
            .eq('booking_id', id)
            .single();
        if (calEvent?.google_event_id) {
            await (0, google_calendar_js_1.deleteCalendarEvent)(calEvent.google_event_id);
            calendarNote = ' | 📅 Agenda supprimé';
        }
    }
    catch {
        calendarNote = ' | ⚠️ Agenda non supprimé';
    }
    const { error } = await supabase_js_1.supabase.from('bookings').delete().eq('id', id);
    if (error)
        return `Erreur suppression: ${error.message}`;
    return `✅ Réservation ${id} supprimée définitivement${calendarNote}`;
}
async function financialReport(input) {
    ...
    return (0, finance_js_1.formatFinancialReport)(report);
}
async function storeDocument(input) {
    const { data, error } = await supabase_js_1.supabase
        ...
        extracted_data: input['extracted_data'] ? (() => { try {
            return JSON.parse(input['extracted_data']);
        }
        catch {
            return null;
        } })() : null,
        ...
        .single();
    if (error)
        return `Erreur stockage document: ${error.message}`;
    const doc = data;
...
async function readSiteFile(input) {
    const result = await (0, github_js_1.getFileContent)(input['path'], 'autolux-location');
    if (!result)
        return `Fichier non trouvé: ${input['path']}`;
    return result.content;
}
async function updateSiteFile(input) {
    const result = await (0, github_js_1.updateFile)(input['path'], input['content'], input['message'], 'autolux-location');
    if (!result)
        return `Erreur: impossible de mettre à jour ${input['path']}`;
    return `✅ Fichier mis à jour: ${input['path']} (commit: ${result.commitSha})`;
}
async function learnRuleTool(input) {
    const result = await (0, claude_api_js_1.learnRule)(input['instruction']);
...
async function rememberInfo(input) {
    const category = input['category'] ?? 'fact';
    const content = input['content'];
    const domain = CATEGORY_TO_DOMAIN[category] ?? 'note';
    // Normalized SHA256 hash → same fact with punctuation/case variations = same key = UPDATE not INSERT
    const key = (0, memory_engine_js_1.computeMemoryKey)(content, domain);
    // Write to modern memory_facts first
    const modernResult = await (0, memory_engine_js_1.writeMemory)({ key, value: content, domain, source: 'remember_info' });
    // Always write legacy ibrahim_memory (don't break old recall flow)
    const { error: legacyError } = await supabase_js_1.supabase
        .from('ibrahim_memory')
...
async function recallMemory(input) {
    ...
    if (input['query'])
        query = query.ilike('content', `%${input['query']}%`);
    const { data, error } = await query;
    if (error)
        return `Erreur recall: ${error.message}`;
    if (!data?.length)
...
async function getWeather(input) {
    ...
    if (!city || city.toLowerCase().includes('oran')) {
        const data = await (0, web_search_js_1.getOranWeather)();
        return JSON.stringify(data);
    }
    // Avec ville → geocoding Open-Meteo puis météo
    try {
        const geoQuery = country ? `${city}, ${country}` : city;
        const geoResp = await axios_1.default.get(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(geoQuery)}&count=1&language=fr&format=json`, { timeout: 8_000 });
        const results = geoResp.data.results;
        if (!results?.length)
            return `❌ Ville introuvable: ${geoQuery}`;
        const loc = results[0];
        const timezone = encodeURIComponent(loc.timezone ?? 'auto');
        const wxResp = await axios_1.default.get(`https://api.open-meteo.com/v1/forecast?latitude=${loc.latitude}&longitude=${loc.longitude}&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weathercode,is_day&timezone=${timezone}`, { timeout: 8_000 });
        const c = wxResp.data.current;
        const WMO = {
        ...
        const wmo = WMO[c.weathercode] ?? { label: 'Inconnu', icon: '❓' };
...
async function getNews(input) {
    const source = input['source'] || 'algerie';
    try {
        const query = source === 'monde' ? 'actualités monde today' : 'actualités Algérie aujourd\'hui';
        const encoded = encodeURIComponent(query);
        const resp = await axios_1.default.get(`https://news.google.com/rss/search?q=${encoded}&hl=fr&gl=DZ&ceid=DZ:fr`, { timeout: 8000 });
...
async function githubReadFile(input) {
    const repo = input['repo'] || 'ibrahim';
    const path = input['path'];
    const result = await (0, github_js_1.getFileContent)(path, repo);
    if (!result)
...
async function githubWriteFile(input) {
    ...
    const path = input['path'];
    const content = input['content'];
    const message = input['message'] || 'update';
    const result = await (0, github_js_1.updateFile)(path, content, message, repo);
    if (!result)
        return `Erreur: impossible de mettre à jour ${path}`;
    return `✅ Fichier mis à jour: ${path} (commit: ${result.commitSha})`;
}
async function githubPatchFile(input) {
    const repo = input['repo'] || 'ibrahim';
    const path = input['path'];
    const oldString = input['old_string'];
    const newString = input['new_string'];
    const message = input['message'] || 'patch: surgical edit';
    if (!path || oldString === undefined || newString === undefined)
        return '❌ repo, path, old_string et new_string sont requis';
    const result = await (0, github_js_1.getFileContent)(path, repo);
    if (!result)
        return `❌ Fichier non trouvé: ${path} dans ${repo}`;
    ...
    const writeResult = await (0, github_js_1.updateFile)(path, newContent, message, repo);
    if (!writeResult)
        return `❌ Impossible de commiter ${path}`;
    const preview = oldString.split('\n')[0].trim().slice(0, 60);
    return `✅ Patch appliqué dans ${path} (commit: ${writeResult.commitSha})\n→ "${preview}..." remplacé avec succès`;
}
async function githubListFiles(input) {
    const repo = input['repo'] || 'ibrahim';
    const path = input['path'] || '';
    const files = await (0, github_js_1.listDirectory)(path, repo);
    if (!files.length)
        return `Répertoire vide ou non trouvé: ${path || '/'}`;
    return files.map(f => `${f.type === 'dir' ? '📁' : '📄'} ${f.path}`).join('\n');
}
async function railwayGetLogs(input) {
    const limit = Number(input['limit'] ?? 50);
    return (0, railway_js_1.getRailwayLogs)(limit);
}
async function supabaseExecute(input) {
    const sql = input['sql'];
    if (!sql)
        return 'SQL manquant';
    ...
    const supabaseToken = env_js_1.env.SUPABASE_ACCESS_TOKEN;
    if (!supabaseToken)
        return 'SUPABASE_ACCESS_TOKEN non configuré dans Railway.';
    try {
        const projectRef = supabaseUrl.match(/https:\/\/([^.]+)\./)?.[1];
        if (!projectRef)
            return 'Impossible d\'extraire le project ref depuis SUPABASE_URL';
        const resp = await axios_1.default.post(`https://api.supabase.com/v1/projects/${projectRef}/database/query`, { query: sql }, {
            headers: {
...
async function netlifyDeploy(input) {
    const siteId = input['site_id'] || 'fik-conciergerie-oran';
    const ok = await (0, github_js_1.triggerNetlifyDeploy)(siteId);
    return ok ? `✅ Déploiement Netlify déclenché pour: ${siteId}` : `❌ Échec du déploiement Netlify pour: ${siteId}`;
}
// ─── VERCEL TOOLS ─────────────────────────────────────────────────────────────
async function vercelGetDeploymentsTool(input) {
    const project = input['project'] ?? 'autolux-location';
    const deploys = await (0, github_js_1.vercelGetDeployments)(project);
    if (!deploys.length) {
...
async function vercelGetDeploymentLogsTool(input) {
    const deploymentId = input['deployment_id'];
    if (!deploymentId)
        return '❌ deployment_id requis (obtiens-le via vercel_get_deployments)';
    return (0, github_js_1.vercelGetDeploymentLogs)(deploymentId);
}
async function vercelCheckUrlTool(input) {
    const url = input['url'] ?? 'https://autolux-location.vercel.app';
    const result = await (0, github_js_1.vercelCheckUrl)(url);
    if (result.ok)
        return `✅ ${url} → HTTP ${result.status} (OK)`;
    if (result.status === 0)
        return `❌ ${url} → inaccessible (timeout ou DNS)`;
    return `⚠️ ${url} → HTTP ${result.status} (${result.status === 404 ? 'page non trouvée' : result.status === 500 ? 'erreur serveur' : 'voir logs'})`;
}
async function vercelRedeployTool(input) {
    const deploymentId = input['deployment_id'];
    if (!deploymentId)
        return '❌ deployment_id requis (obtiens-le via vercel_get_deployments)';
    return (0, github_js_1.vercelRedeploy)(deploymentId);
}
// ─── PHASE 13 — APPRENTISSAGE CONTINU ─────────────────────────────────────
async function recordFeedbackTool(input, sessionId) {
    ...
    const feedback = await (0, feedback_system_js_1.recordFeedback)({
...
async function getMonthlyImprovementReportTool(input) {
    const now = new Date();
    const year = input['year'] ? Number(input['year']) : now.getFullYear();
    const month = input['month'] ? Number(input['month']) : now.getMonth() + 1;
    const report = await (0, improvement_report_js_1.generateMonthlyReport)(year, month);
    return (0, improvement_report_js_1.formatReportForKouider)(report);
}
async function getLearningEvolutionTool(input) {
    const months = input['months'] ? Number(input['months']) : 6;
    const evolution = await (0, improvement_report_js_1.getEvolutionReport)(months);
...
async function getKouiderPreferencesTool() {
    const prefs = await (0, feedback_system_js_1.getKouiderPreferences)();
...
async function sendWhatsAppToClient(input) {
    const phone = input['phone'];
    const message = input['message'];
    if (!phone || !message)
        return '❌ phone et message sont requis';
    const ok = await (0, whatsapp_js_1.sendWhatsApp)(phone, message);
    return ok
        ? `✅ Message WhatsApp envoyé à ${phone}`
        : `❌ Échec envoi WhatsApp à ${phone} (Twilio non configuré ?)`;
}
async function checkCarAvailability(input) {
    const startDate = input['start_date'];
    const endDate = input['end_date'];
    const carId = input['car_id'];
    if (!startDate || !endDate)
        return '❌ start_date et end_date sont requis';
    const overlappingQuery = supabase_js_1.supabase
        .from('bookings')
        .select('car_id')
    ...
    const busyCarIds = new Set((overlapping ?? []).map((b) => b.car_id));
    let carsQuery = supabase_js_1.supabase.from('cars').select('id, name, base_price, category').eq('available', true);
    if (carId)
        carsQuery = carsQuery.eq('id', carId);
...
async function githubSearchCode(input) {
    const repo = input['repo'] || 'ibrahim';
    const query = input['query'];
    if (!query)
        return 'Query requise';
    return (0, github_js_1.searchCode)(repo, query);
}
async function getClientDocument(input) {
    // Build REST API URL directly (bypass supabase JS type issues)
    const SUPA_URL = env_js_1.env['SUPABASE_URL'];
    const SUPA_KEY = env_js_1.env['SUPABASE_SERVICE_KEY'];
    ...
    const filters = [];
    if (clientName)
        filters.push(`client_name=ilike.*${clientName}*`);
    if (clientPhone)
        filters.push(`client_phone=ilike.*${clientPhone}*`);
    if (docType)
        filters.push(`type=eq.${docType}`);
    const filterStr = filters.length ? `&${filters.join('&')}` : '';
    const restUrl = `${SUPA_URL}/rest/v1/client_documents?select=${select}&order=created_at.desc&limit=5${filterStr}`;
    const { default: axiosModule } = await Promise.resolve().then(() => __importStar(require('axios')));
    let docs = [];
    try {
        const { data } = await axiosModule.get(restUrl, {
    ...
    if (docs.length === 0) {
        ...
        try {
            const { data: all } = await axiosModule.get(`${SUPA_URL}/rest/v1/client_documents?select=client_name,type&limit=30`, { headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}` }, timeout: 8_000 });
            const total = all?.length ?? 0;
            if (!total)
                return 'TABLE VIDE — aucun document enregistré dans la base client_documents.';
    ...
    if (field) {
        ...
        if (field === 'client_phone' || field === 'phone')
            return doc.client_phone ?? '❌ Téléphone non renseigné';
        if (field === 'file_url' || field === 'url')
            return doc.file_url ?? '❌ URL non disponible';
    ...
    const chatId = env_js_1.env.TELEGRAM_CHAT_ID ? Number(env_js_1.env.TELEGRAM_CHAT_ID) : null;
    const sentUrls = [];
    for (const d of docs) {
        ...
        try {
            ...
            await (0, telegram_js_2.sendPhotoBuffer)(chatId, buf, caption);
...
async function webSearch(input) {
    ...
    if (!query)
        return 'Query requise';
    const result = await (0, web_search_provider_js_1.multiProviderWebSearch)(query);
...
async function fetchUrl(input) {
    ...
    try {
        ...
        const { data } = await axios_1.default.get(`https://r.jina.ai/${encoded}`, {
            headers: (0, web_search_provider_js_1.jinaAuthHeaders)(),
            timeout: 20_000,
        });
...
async function scheduleReminder(input, sessionId) {
    const message = input['message'];
    const delayMinutes = input['delay_minutes'];
    const atTime = input['at_time'];
    const tzInput = input['timezone'];
    if (!message)
        return JSON.stringify({ error: '❌ message requis', source_channel: 'unknown', request_id: 'none', message: '' });
    ...
    const [sessionTzRaw, globalTzRaw] = await Promise.all([
        sessionTzKey ? queue_js_1.redis.get(sessionTzKey) : Promise.resolve(null),
        queue_js_1.redis.get('user:tz'),
    ]);
    const sessionTz = (sessionTzRaw && (0, timezone_js_1.isValidTimezone)(sessionTzRaw)) ? sessionTzRaw : null;
    const globalTz = (globalTzRaw && (0, timezone_js_1.isValidTimezone)(globalTzRaw)) ? globalTzRaw : null;
    const resolved = (0, timezone_js_1.resolveTimezone)(tzInput ?? null, sessionTz ?? globalTz ?? null);
    ...
    if (delayMinutes !== undefined && Number(delayMinutes) > 0) {
        ...
        const parsed = (0, timezone_js_1.parseLocalHHMM)(atTime, timezone);
        if (!parsed) {
    ...
    const utcOffset = (0, timezone_js_1.getUTCOffsetString)(timezone, remindAt);
    const localTimeISO = (0, timezone_js_1.toLocalISO)(remindAt, timezone);
    // ── 4. Idempotency key ───────────────────────────────────────────────
    ...
    const idempotency_key = crypto_1.default
        .createHash('sha256')
        .update(idemRaw)
        .digest('hex')
        .slice(0, 32);
    ...
    const dbExisting = await (0, reminders_js_1.findByDedupKey)(idempotency_key);
    if (dbExisting) {
        return JSON.stringify({
            status: 'duplicate_blocked',
            idempotency_key,
    ...
    await queue_js_1.redis.set(redisKey, jobId, 'EX', IDEM_TTL_SEC);
    // ── 8. Human delay ───────────────────────────────────────────────────
    ...
    console.log(`[schedule_reminder] CREATED req=${request_id} db_id=${dbRow.id} ` +
...
// ─── GOOGLE CALENDAR ─────────────────────────────────────────────────────────
async function createCalendarEventTool(input) {
    const bookingId = input['booking_id'];
    const clientName = input['client_name'];
    const carName = input['car_name'];
    const startDate = input['start_date'];
    const endDate = input['end_date'];
    ...
    if (!eventId)
        return '❌ Impossible de créer l\'événement Google Calendar. Vérifie GOOGLE_SERVICE_ACCOUNT_JSON dans Railway.';
    return `✅ Événement créé dans Google Agenda!\n📅 ${clientName} — ${carName}\n📆 ${startDate} → ${endDate}\n🔗 Event ID: ${eventId}`;
}
async function syncCalendarTool() {
    const count = await (0, google_calendar_js_1.syncPendingBookings)();
    if (count === 0)
        return '✅ Tout est déjà synchronisé — aucune réservation manquante dans l\'agenda.';
    return `✅ ${count} réservation(s) ajoutée(s) dans Google Agenda!`;
}
async function listCalendarEventsTool(input) {
    const maxResults = Number(input['max_results'] ?? 20);
    const events = await (0, google_calendar_js_1.listUpcomingEvents)(maxResults);
    if (!events.length)
        return 'Aucun événement à venir dans Google Agenda.';
    return `📅 ${events.length} événement(s) dans Google Agenda:\n${events.map(e => `- ${e.summary} | ${e.start.dateTime?.slice(0, 10) ?? '?'} → ${e.end.dateTime?.slice(0, 10) ?? '?'}`).join('\n')}`;
}
async function deleteCalendarEventTool(input) {
    const googleEventId = input['google_event_id'];
    if (!googleEventId)
        return '❌ google_event_id requis';
    const ok = await (0, google_calendar_js_1.deleteCalendarEvent)(googleEventId);
    if (!ok)
        return `❌ Impossible de supprimer l'événement ${googleEventId}`;
...
async function searchImages(input) {
    ...
    if (!query)
        return '❌ Query requise';
    const PEXELS_KEY = env_js_1.env.PEXELS_API_KEY;
    if (!PEXELS_KEY) {
...
async function sendTelegramMessage(input) {
    if (!env_js_1.env.TELEGRAM_CHAT_ID)
        return '❌ TELEGRAM_CHAT_ID non configuré sur le serveur';
    const chatId = Number(env_js_1.env.TELEGRAM_CHAT_ID);
    const message = input['message'] ?? '';
    const photoUrl = input['photo_url'];
    const docUrl = input['document_url'];
    const caption = input['caption'] ?? message;
    try {
        if (photoUrl) {
            await (0, telegram_js_2.sendPhoto)(chatId, photoUrl, caption);
            if (message && message !== caption)
                await (0, telegram_js_2.sendMessage)(chatId, message);
            return `✅ Photo envoyée sur Telegram${message ? ` avec message: "${message}"` : ''}`;
        }
        if (docUrl) {
            await (0, telegram_js_2.sendDocument)(chatId, docUrl, caption);
            if (message && message !== caption)
                await (0, telegram_js_2.sendMessage)(chatId, message);
...
async function generateVoucherTool(input, sessionId) {
    const bookingId = input['booking_id'];
    if (!bookingId)
        return '❌ booking_id requis';
    const { url, clientName, buffer } = await (0, generate_voucher_js_1.generateReservationVoucher)(bookingId);
    const filename = `BON_${clientName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
    const caption = `📄 Bon de réservation — ${clientName}`;
    const sendPDF = async (chatId) => {
        const botBase = `https://api.telegram.org/bot${env_js_1.env.TELEGRAM_BOT_TOKEN ?? ''}`;
        ...
        const resp = await axios_1.default.post(`${botBase}/sendDocument`, form, { headers: form.getHeaders(), maxBodyLength: Infinity, maxContentLength: Infinity });
        if (!resp.data.ok) {
    ...
    if (env_js_1.env.TELEGRAM_CHAT_ID) {
        await sendPDF(Number(env_js_1.env.TELEGRAM_CHAT_ID)).catch((e) => console.error('[voucher] voice send failed:', e instanceof Error ? e.message : String(e)));
    }
    return `✅ Bon de réservation PDF généré pour ${clientName} ! 📄\n${url}`;
}
async function getFleetStatus() {
    const today = new Date().toISOString().slice(0, 10);
    const { data: cars, error: carErr } = await supabase_js_1.supabase
        .from('cars')
        .select('id, name, category, base_price, available')
        .order('name');
    if (carErr || !cars?.length)
        return '❌ Impossible de récupérer la flotte.';
    const { data: activeBookings } = await supabase_js_1.supabase
        .from('bookings')
        .select('car_id, client_name, client_phone, start_date, end_date, payment_status, paid_amount, final_price')
        .in('status', ['CONFIRMED', 'ACTIVE'])
        .lt('start_date', today)
        .gte('end_date', today);
    const { data: upcomingBookings } = await supabase_js_1.supabase
        .from('bookings')
    ...
    for (const b of (activeBookings ?? []))
        activeMap.set(b.car_id, b);
    const upcomingMap = new Map();
    for (const b of (upcomingBookings ?? [])) {
        if (!upcomingMap.has(b.car_id))
            upcomingMap.set(b.car_id, b);
    }
    const lines = [`🚗 *ÉTAT FLOTTE — ${today}*`, '─'.repeat(35)];
    ...
    for (const car of cars) {
        ...
        if (active) {
            ...
            available++;
            const nextLine = upcoming ? ` → prochain: ${upcoming.client_name} le ${upcoming.start_date}` : '';
...
async function rateClient(input) {
    const bookingId = input['booking_id'];
    ...
    const { data: booking, error } = await supabase_js_1.supabase
        ...
        .single();
    if (error || !booking)
        return `❌ Réservation introuvable: ${bookingId}`;
    const stars = '⭐'.repeat(rating) + '☆'.repeat(5 - rating);
    const notes = comment ? `${stars} — ${comment}` : stars;
    const { error: updErr } = await supabase_js_1.supabase
        .from('bookings')
        .update({ notes: `[NOTE CLIENT] ${notes}` })
        .eq('id', bookingId);
    if (updErr)
        return `❌ Erreur: ${updErr.message}`;
    return `✅ Client ${booking.client_name} noté ${stars}${comment ? ` — "${comment}"` : ''}`;
}
async function getLateReturns() {
    ...
    if (!data?.length)
        return '✅ Aucun véhicule en retard de retour.';
...
function chatIdFromSession(sessionId) {
    if (sessionId?.startsWith('telegram_'))
        return sessionId.slice('telegram_'.length);
 ...
 */
function sanitizeCustomScript(raw) {
    if (!raw)
        return undefined;
    return raw
        .trim()
...
async function runTikTokResearchTool(sessionId, input) {
    const chatId = chatIdFromSession(sessionId);
    const carFocus = input?.['car_focus'];
    const extraTags = input?.['hashtags'];
    const { data: carsRaw } = await supabase_js_1.supabase.from('cars').select('*').eq('available', true);
    const cars = (carsRaw ?? []);
    if (cars.length === 0) {
        return '⚠️ Aucune voiture disponible pour la recherche marketing.';
    }
    const focusLabel = carFocus ? ` — focus: ${carFocus}` : '';
    await (0, telegram_js_1.sendMessage)(chatId, `🔍 *Dzaryx Marketing*\nRecherche TikTok lancée${focusLabel}... ⏳`);
    const report = await (0, market_research_js_1.runTikTokMarketResearch)(cars, carFocus, extraTags);
...
async function createMarketingVideoTool(input, sessionId) {
    ...
    const pricing = (0, pricing_js_1.getPricingForVehicle)(car.name);
    ...
    if (customScript) {
        ...
        const sr = await (0, claude_api_js_1.chat)([{
                role: 'user',
...
Accrocheur, prix + "Fik Conciergerie Oran" mentionnés, CTA fort. RÉPONDS UNIQUEMENT avec le script, sans guillemets.`,
    ...
    const runwayKey = env_js_1.env.RUNWAY_API_KEY;
    ...
    if (runwayKey || falKey) {
        ...
        await (0, telegram_js_1.sendMessage)(chatId, `🎬 *Vidéo TikTok — ${car.name}*\n_${providerLabel}${backgroundEffect ? ` · fond ${backgroundEffect}` : ''}_\n⏳ 60-240 secondes...`).catch(() => { });
        try {
            // Vérifier que l'image est publiquement accessible
            await axios_1.default.head(car.image_url, { timeout: 8_000 });
            ...
            const resp = await axios_1.default.get(result.url, { responseType: 'arraybuffer', timeout: 60_000 });
            const rawBuf = Buffer.from(resp.data);
            if (!(0, create_marketing_video_js_1.isValidMp4Buffer)(rawBuf)) {
            ...
            await (0, telegram_js_1.sendMessage)(chatId, `⚠️ _IA vidéo indisponible (\`${msg.slice(0, 120)}\`) — FFmpeg fallback..._`).catch(() => { });
        }
    }
    // ── Tentative 2 : FFmpeg + ElevenLabs (local, fiable) ────────
    if (!videoBuffer) {
        await (0, telegram_js_1.sendMessage)(chatId, `🎬 *Vidéo TikTok — ${car.name}*\n_Montage FFmpeg HD 1080×1920${backgroundEffect ? ` · fond ${backgroundEffect}` : ''}_\n⏳ Génération voix + montage...`).catch(() => { });
        try {
            // Utilise executeCreateMarketingVideo — le module complet avec upload Supabase
            const result = await (0, create_marketing_video_js_1.executeCreateMarketingVideo)({
                car_name: car.name,
                style: style,
            ...
            await (0, telegram_js_1.sendMessage)(chatId, `❌ _FFmpeg aussi échoué:_ \`${msg.slice(0, 200)}\`\n_Envoi photo + voix..._`).catch(() => { });
        }
    }
    // ── Voix ElevenLabs ──────────────────────────────────────────
    ...
    if (videoBuffer && audioBuffer) {
        try {
            const merged = await (0, create_marketing_video_js_1.mergeVideoWithAudio)(videoBuffer, audioBuffer);
            if ((0, create_marketing_video_js_1.isValidMp4Buffer)(merged)) {
                videoBuffer = merged;
    ...
    const pendingId = await (0, approval_store_js_1.savePendingVideo)({
    ...
    (0, video_session_store_js_1.saveVideoSession)({
        carName: car.name,
    ...
    let carImgBuf = null;
    try {
        const imgResp = await axios_1.default.get(car.image_url, { responseType: 'arraybuffer', timeout: 20_000 });
        carImgBuf = Buffer.from(imgResp.data);
    }
    catch {
    ...
    if (videoBuffer) {
        try {
            await (0, telegram_js_1.sendVideoBuffer)(chatId, videoBuffer, approvalMsg);
            ...
            if (carImgBuf) {
                try {
                    await (0, telegram_js_1.sendPhotoBuffer)(chatId, carImgBuf, approvalMsg);
    ...
    if (!videoBuffer && audioBuffer) {
        await (0, telegram_js_1.sendVoiceBuffer)(chatId, audioBuffer).catch(() => { });
    }
    let resultMsg;
...
async function editMarketingVideoTool(input, sessionId) {
    const chatId = chatIdFromSession(sessionId);
    const session = (0, video_session_store_js_1.getLatestVideoSession)();
    ...
    await (0, telegram_js_1.sendMessage)(chatId, `✏️ *Modification vidéo — ${session.carName}*\n_"${modification.slice(0, 80)}"_\n⏳ En cours...`).catch(() => { });
    // Re-generate voice if script/tone changed
    let audioBuffer = session.audioBuffer;
    if (!isVideoOnly) {
        const newScript = input['new_script'] ?? session.script;
        ...
        audioBuffer = await (0, dispatcher_js_1.synthesizeVoice)(scriptToUse).catch(() => session.audioBuffer);
    }
    ...
    (0, video_session_store_js_1.saveVideoSession)({
        ...
        audioBuffer,
        script: input['new_script'] ?? session.script,
        provider,
        prompt: modification,
    });
    // Send to Telegram
    ...
    if (finalVideo) {
        await (0, telegram_js_1.sendVideoBuffer)(chatId, finalVideo, approvalMsg).catch(async () => {
            await (0, telegram_js_1.sendMessage)(chatId, approvalMsg).catch(() => { });
        });
    }
    else {
        await (0, telegram_js_1.sendMessage)(chatId, approvalMsg).catch(() => { });
...
async function regenerateVoiceTool(input, sessionId) {
    const chatId = chatIdFromSession(sessionId);
    const session = (0, video_session_store_js_1.getLatestVideoSession)();
    if (!session) {
        return '⚠️ Aucune vidéo en mémoire. Génère d\'abord une vidéo.';
    }
    const newScript = input['script'] ?? session.script;
    const tone = input['tone'] ?? '';
    const tonePrefix = tone === 'professionnel' ? 'Ton professionnel et sérieux. ' :
        tone === 'dynamique' ? 'Ton dynamique et énergique. ' :
            tone === 'chaleureux' ? 'Ton chaleureux et accueillant. ' :
                tone === 'commercial' ? 'Ton commercial percutant. ' : '';
    await (0, telegram_js_1.sendMessage)(chatId, `🎙️ *Nouvelle voix en cours...*\n_Ton: ${tone || 'standard'}_`).catch(() => { });
    const audioBuffer = await (0, dispatcher_js_1.synthesizeVoice)(tonePrefix + newScript);
    if (!audioBuffer)
        return '❌ ElevenLabs indisponible — vérifie ELEVENLABS_API_KEY dans Railway.';
    // Merge with existing video if available
    let finalVideo = session.videoBuffer;
    if (session.videoBuffer && audioBuffer) {
        try {
            const merged = await (0, create_marketing_video_js_1.mergeVideoWithAudio)(session.videoBuffer, audioBuffer);
            if ((0, create_marketing_video_js_1.isValidMp4Buffer)(merged))
                finalVideo = merged;
        }
    ...
    // Update session
    (0, video_session_store_js_1.saveVideoSession)({ ...session, audioBuffer, videoBuffer: finalVideo, script: newScript });
    // Send
    if (finalVideo) {
        await (0, telegram_js_1.sendVideoBuffer)(chatId, finalVideo, `🎙️ *Voix modifiée — ${session.carName}*\n_Script: "${newScript.slice(0, 100)}_"\n\n✅ *Oke* pour publier | ❌ *Non* pour annuler`).catch(async () => {
            await (0, telegram_js_1.sendVoiceBuffer)(chatId, audioBuffer).catch(() => { });
        });
    }
    else {
...
async function createScenarioVideoTool(input, sessionId) {
    const chatId = chatIdFromSession(sessionId);
    const scenario = input['scenario'] ?? 'airport_arrival';
    const carName = input['car_name'];
    const falKey = env_js_1.env.FAL_KEY;
    const runwayKey = env_js_1.env.RUNWAY_API_KEY;
    if (!falKey && !runwayKey) {
    ...
    const car = carName
        ...
        : carsWithImage[0];
    if (!car)
        return '⚠️ Aucune voiture avec photo disponible dans la flotte.';
    const pricing = (0, pricing_js_1.getPricingForVehicle)(car.name);
    ...
    await (0, telegram_js_1.sendMessage)(chatId, `🎬 *Scénario : ${sc.label}*\n_${car.name} — ${priceDisplay}_\n⏳ Génération Runway/Kling en cours...`).catch(() => { });
    ...
    await (0, telegram_js_1.sendMessage)(chatId, brief).catch(() => { });
    // Generate video
    let videoBuffer = null;
    ...
    try {
        ...
        await (0, telegram_js_1.sendMessage)(chatId, `⚠️ Vidéo IA indisponible, photo envoyée à la place.`).catch(() => { });
    }
    // Generate voice
    const audioBuffer = await (0, dispatcher_js_1.synthesizeVoice)(sc.voiceScript).catch(() => null);
    // Merge
    let finalVideo = videoBuffer;
    if (videoBuffer && audioBuffer) {
        try {
    ...
    if (finalVideo) {
        await (0, telegram_js_1.sendVideoBuffer)(chatId, finalVideo, approvalMsg).catch(async () => {
            await (0, telegram_js_1.sendPhoto)(chatId, car.image_url, approvalMsg).catch(() => { });
        });
    }
...
async function createVideoProjectTool(input, sessionId) {
    const chatId = chatIdFromSession(sessionId);
    const scenario = input['scenario'] ?? 'client_search';
    const carName = input['car_name'];
    const style = input['style'] ?? 'tiktok';
    const falKey = env_js_1.env.FAL_KEY;
    const runwayKey = env_js_1.env.RUNWAY_API_KEY;
    if (!falKey && !runwayKey) {
    ...
    // ── Build storyboard ───────────────────────────────────────────────────────
    const storyboards = {
        client_search: (0, video_project_store_js_1.buildClientSearchStoryboard)(car.name, priceDisplay),
        airport_arrival: (0, video_project_store_js_1.buildAirportArrivalStoryboard)(car.name, priceDisplay),
        fleet_reveal: (0, video_project_store_js_1.buildFleetRevealStoryboard)(car.name, priceDisplay),
        corniche_drive: (0, video_project_store_js_1.buildCornicheDriveStoryboard)(car.name, priceDisplay),
    };
    const board = storyboards[scenario] ?? storyboards['client_search'];
    // ── Send brief ─────────────────────────────────────────────────────────────
    const totalDur = board.scenes.reduce((s, sc) => s + sc.duration, 0);
    const sceneList = board.scenes.map((sc, i) => `*${i + 1}. ${sc.label}* (${sc.duration}s) — ${sc.overlayText ?? sc.type}`).join('\n');
    const brief = [
    ...
    await (0, telegram_js_1.sendMessage)(chatId, brief).catch(() => { });
    // ── Save project ───────────────────────────────────────────────────────────
    const project = (0, video_project_store_js_1.saveVideoProject)({
        title: board.title,
        scenario,
        carName: car.name,
        carImageUrl: car.image_url,
    ...
    // ── Generate all scenes ────────────────────────────────────────────────────
    const tmpDir = await Promise.resolve().then(() => __importStar(require('os'))).then(o => Promise.resolve().then(() => __importStar(require('fs/promises'))).then(f => f.mkdtemp(o.tmpdir() + '/dzaryx-proj-')));
    const fontPath = await (0, scene_assembler_js_1.ensureSceneFont)();
    const scenePaths = [];
    ...
    try {
        for (let i = 0; i < board.scenes.length; i++) {
            ...
            if (sc.type.startsWith('ui_')) {
                ...
                await (0, telegram_js_1.sendMessage)(chatId, `🖥️ _Scène ${i + 1}/${board.scenes.length} — ${sc.label} (FFmpeg)_`).catch(() => { });
                await (0, scene_assembler_js_1.generateUISceneFile)(sc, outPath, fontPath);
                scenePaths.push(outPath);
            }
            else {
                // Car scene — Runway/Kling
                await (0, telegram_js_1.sendMessage)(chatId, `🎬 _Scène ${i + 1}/${board.scenes.length} — ${sc.label} (Runway/Kling ~90s)_`).catch(() => { });
                let carClipBuffer = null;
                try {
                    await axios_1.default.head(car.image_url, { timeout: 8_000 });
                    ...
                    const result = await generateVehicleVideo({
                        ...
                        userPrompt: prompt,
                        carName: car.name,
                        duration: Math.min(sc.duration, 5),
                        falKey,
                        runwayKey,
                        forceProvider: 'auto',
                        sceneTransformation: true,
                    ...
                    if ((0, create_marketing_video_js_1.isValidMp4Buffer)(raw)) {
                        carClipBuffer = raw;
                ...
                if (!carClipBuffer) {
                    await (0, telegram_js_1.sendMessage)(chatId, `⚠️ _Scène ${i + 1} IA indisponible — image statique utilisée_`).catch(() => { });
                    // Create a simple static scene from car image using FFmpeg
                    try {
                        const imgBuf = await axios_1.default.get(car.image_url, { responseType: 'arraybuffer', timeout: 20_000 })
                            .then(r => Buffer.from(r.data));
                        const imgPath = `${tmpDir}/car_${i}.jpg`;
                        await Promise.resolve().then(() => __importStar(require('fs/promises'))).then(f => f.writeFile(imgPath, imgBuf));
                        ...
                        carClipBuffer = await Promise.resolve().then(() => __importStar(require('fs/promises'))).then(f => f.readFile(outPath));
                    }
                    catch (fbErr) {
                ...
                if (sc.overlayText && carClipBuffer) {
                    const withOverlay = await (0, scene_assembler_js_1.addOverlayToClip)(carClipBuffer, sc.overlayText, fontPath)
                        .catch(() => carClipBuffer);
                    await Promise.resolve().then(() => __importStar(require('fs/promises'))).then(f => f.writeFile(outPath, withOverlay));
                }
                else if (carClipBuffer) {
                    await Promise.resolve().then(() => __importStar(require('fs/promises'))).then(f => f.writeFile(outPath, carClipBuffer));
                }
                scenePaths.push(outPath);
            }
        }
        ...
        // ── Generate voice ─────────────────────────────────────────────────────
        await (0, telegram_js_1.sendMessage)(chatId, '🎙️ _Génération voix off ElevenLabs..._').catch(() => { });
        const audioBuffer = await (0, dispatcher_js_1.synthesizeVoice)(board.voiceScript).catch(() => null);
        // ── Assemble ──────────────────────────────────────────────────────────
        await (0, telegram_js_1.sendMessage)(chatId, '🎞️ _Assemblage final FFmpeg..._').catch(() => { });
        const finalBuffer = await (0, scene_assembler_js_1.concatScenesWithVoice)(scenePaths, audioBuffer, tmpDir);
        ...
        (0, video_project_store_js_1.updateVideoProject)(project.id, { finalBuffer, audioBuffer, pendingId, provider: usedProvider });
        (0, video_session_store_js_1.saveVideoSession)({
            carName: car.name,
            carImageUrl: car.image_url,
            carId: car.id,
        ...
        await (0, telegram_js_1.sendVideoBuffer)(chatId, finalBuffer, approvalMsg).catch(async () => {
            await (0, telegram_js_1.sendPhoto)(chatId, car.image_url, approvalMsg).catch(() => { });
        });
        return `✅ Projet vidéo "${board.title}" (${scenePaths.length} scènes, ${usedProvider}) envoyé sur Telegram ↑ (ID: ${pendingId}).`;
    }
    finally {
        await Promise.resolve().then(() => __importStar(require('fs/promises'))).then(f => f.rm(tmpDir, { recursive: true, force: true })).catch(() => { });
    }
}
function runFFmpegForProject(args) {
    // @ts-ignore
...
async function mergeVideosTool(_input, sessionId) {
    const chatId = chatIdFromSession(sessionId);
    const fileIds = (0, video_buffer_js_1.getVideoBuffer)(sessionId ?? '');
    if (fileIds.length < 2) {
    ...
    await (0, telegram_js_1.sendMessage)(chatId, `🎬 *Fusion de ${fileIds.length} vidéos en cours...*\n_Normalisation + montage_ ⏳`);
    // Download all videos from Telegram
    const { downloadFile: downloadTelegramFile } = await Promise.resolve().then(() => __importStar(require('./telegram.js')));
    const buffers = [];
    for (const fileId of fileIds) {
        const buf = await downloadTelegramFile(fileId);
        if (!buf) {
            await (0, telegram_js_1.sendMessage)(chatId, `⚠️ Impossible de télécharger la vidéo (ID: ${fileId}) — elle a peut-être expiré.`);
            return `⚠️ Échec téléchargement d'une vidéo.`;
        }
        buffers.push(buf);
    }
    const merged = await (0, video_creator_js_1.mergeVideos)(buffers).catch(async (err) => {
        const msg = err instanceof Error ? err.message : String(err);
        console.error('[tool:merge_videos] failed:', msg);
        await (0, telegram_js_1.sendMessage)(chatId, `⚠️ Fusion échouée: ${msg.slice(0, 120)}`);
        return null;
    });
    if (!merged)
        return '⚠️ Fusion des vidéos échouée.';
    (0, video_buffer_js_1.clearVideoBuffer)(sessionId ?? '');
    const caption = `🎬 *Vidéo fusionnée — ${fileIds.length} clips*\n\nFusionnée par Dzaryx ✨`;
    await (0, telegram_js_1.sendVideoBuffer)(chatId, merged, caption).catch(async (err) => {
        console.error('[tool] merge sendVideoBuffer failed:', err instanceof Error ? err.message : err);
        await (0, telegram_js_1.sendMessage)(chatId, `⚠️ Upload vidéo fusionnée échoué: ${err instanceof Error ? err.message : String(err)}`);
    });
...
// ── VEILLE CONCURRENTIELLE ────────────────────────────────────
// ════════════════════════════════════════════════════════════════
async function jSearch(query, maxChars = 1500) {
    const result = await (0, web_search_provider_js_1.multiProviderWebSearch)(query);
    return result.text.slice(0, maxChars);
}
async function jFetch(url, maxChars = 2500) {
    try {
        const { data } = await axios_1.default.get(`https://r.jina.ai/${encodeURIComponent(url)}`, {
            headers: (0, web_search_provider_js_1.jinaAuthHeaders)(),
            timeout: 20_000,
        });
...
async function apifyRun(actorId, inputPayload) {
    const apiKey = env_js_1.env.APIFY_API_KEY;
    if (!apiKey)
        return [];
    const runResp = await axios_1.default.post(`https://api.apify.com/v2/acts/${actorId}/runs?token=${apiKey}`, inputPayload, { timeout: 30_000 });
    ...
    for (let i = 0; i < 24; i++) {
        ...
        const statusResp = await axios_1.default.get(`https://api.apify.com/v2/actor-runs/${runId}?token=${apiKey}`, { timeout: 10_000 });
    ...
    if (!datasetId)
        return [];
    const itemsResp = await axios_1.default.get(`https://api.apify.com/v2/datasets/${datasetId}/items?token=${apiKey}&limit=40`, { timeout: 15_000 });
    return itemsResp.data ?? [];
}
function formatTikTokItems(items) {
    if (!items.length)
        return 'Aucun résultat TikTok trouvé.';
    const byAuthor = {};
    ...
    for (const [handle, videos] of Object.entries(byAuthor)) {
        ...
        for (const v of videos.slice(0, 6)) {
            ...
            const tags = (v.hashtags ?? []).map((h) => `#${h.name ?? h}`).join(' ');
            output += `  • "${desc}" ${tags}\n`;
...
async function analyzeCompetitors(input, sessionId) {
    const competitor = input['competitor'];
    const extraHashtags = input['hashtags'];
    const carFocus = input['car_focus'];
    const makeVideo = input['generate_counter_video'];
    const chatId = chatIdFromSession(sessionId);
    // ── Notification de démarrage ──────────────────────────────
    const focusLabel = competitor ? `_Cible: ${competitor}_` : carFocus ? `_Focus: ${carFocus}_` : '_Scan général: location voiture Oran_';
    await (0, telegram_js_1.sendMessage)(chatId, `🕵️ *Veille concurrentielle lancée*\n${focusLabel}\n⏳ 10 recherches en cours...`).catch(() => { });
    ...
    const pricing = (0, pricing_js_1.formatPricingTable)();
    // ── Analyse Claude avec les données collectées ─────────────
    const analysis = await (0, claude_api_js_1.chat)([{
            role: 'user',
...
(15-20 sec en français, exploite une lacune détectée)`,
    ...
    if (makeVideo) {
        await (0, telegram_js_1.sendMessage)(chatId, `${analysis.text}\n\n⏳ _Création de la contre-pub en cours..._`);
        const { data: cars } = await supabase_js_1.supabase.from('cars').select('*').eq('available', true).limit(1);
    ...
    await (0, telegram_js_1.sendMessage)(chatId, analysis.text).catch(() => { });
    return analysis.text.substring(0, 3000);
}
async function watchMyTiktok(input) {
    const handle = (input['handle'] ?? 'fikconciergerieoran').replace('@', '');
...
async function publishToSocialsTool(input, sessionId) {
    const { getLatestPendingVideo, getPendingVideoById, approveVideo } = await Promise.resolve().then(() => __importStar(require('../marketing/approval-store.js')));
    const { publishVideo, buildSharePackage } = await Promise.resolve().then(() => __importStar(require('../marketing/social-poster.js')));
    const pendingId = input['pending_id'];
    const video = pendingId ? getPendingVideoById(pendingId) : getLatestPendingVideo();
    if (!video)
        return '❌ Aucune vidéo en attente — génère-en une avec `create_marketing_video` d\'abord.';
    approveVideo(video.id);
    const chatId = chatIdFromSession(sessionId);
    const tiktokConfigured = Boolean((await Promise.resolve().then(() => __importStar(require('../config/env.js')))).env.TIKTOK_ACCESS_TOKEN &&
        (await Promise.resolve().then(() => __importStar(require('../config/env.js')))).env.TIKTOK_OPEN_ID);
    if (tiktokConfigured) {
        await (0, telegram_js_1.sendMessage)(chatId, `🚀 *Publication TikTok en cours...*`).catch(() => { });
        ...
        await (0, telegram_js_1.sendMessage)(chatId, reply).catch(() => { });
        return reply;
    }
    const pkg = buildSharePackage(video);
    await (0, telegram_js_1.sendMessage)(chatId, pkg).catch(() => { });
    return pkg;
}
// ── CODE AGENT AUTONOME ───────────────────────────────────────
// ════════════════════════════════════════════════════════════════
async function executeCodeTaskTool(input, sessionId) {
    const task = input['task'];
    const repo = input['repo'] ?? 'ibrahim';
    const chatId = chatIdFromSession(sessionId);
    if (!task)
        return '❌ task requis — décris ce qui doit être codé';
    // Lance l'agent en arrière-plan (non-bloquant)
    (0, code_agent_js_1.runCodeAgent)(task, chatId, repo).catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
        (0, telegram_js_1.sendMessage)(chatId, `❌ Code Agent crash: ${msg}`).catch(() => { });
    });
...
async function createNewProjectTool(input, sessionId) {
    const clientName = input['client_name'];
    const businessType = input['business_type'];
    const description = input['description'];
    const phone = input['phone'] ?? '';
    const city = input['city'] ?? 'Oran';
    const chatId = chatIdFromSession(sessionId);
...
À la fin, annoncer que le site est prêt et indiquer comment le déployer sur Netlify.`;
    (0, code_agent_js_1.runCodeAgent)(task, chatId, 'ibrahim').catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
        (0, telegram_js_1.sendMessage)(chatId, `❌ Code Agent crash: ${msg}`).catch(() => { });
    });
...
// ════════════════════════════════════════════════════════════════
async function replicateGenerate(model, input, token, maxMs = 120_000) {
...
async function findCarByName(name) {
    const { data: cars } = await supabase_js_1.supabase.from('cars').select('id, name, image_url');
    if (!cars?.length)
        return null;
...
async function falGenerate(modelId, input, falKey, maxMs = 240_000) {
    ...
    while (Date.now() - start < maxMs) {
        await new Promise(r => setTimeout(r, 5000));
        const statusResp = await axios_1.default.get(pollUrl, { headers: { Authorization: `Key ${falKey}` }, timeout: 15_000, validateStatus: () => true });
        const httpStatus = statusResp.status;
        if (httpStatus === 401 || httpStatus === 403)
            throw new Error(`fal.ai: auth rejetée (${httpStatus})`);
        if (httpStatus === 404)
            throw new Error('fal.ai: endpoint introuvable (404)');
        if (httpStatus >= 500)
            throw new Error(`fal.ai: erreur serveur (${httpStatus})`);
        const jobStatus = statusResp.data?.status ?? statusResp.data?.state ?? '';
    ...
    if (!completed)
        throw new Error(`fal.ai: timeout après ${Math.round(maxMs / 1000)}s`);
    // Fetch result via response_url
    const resultResp = await axios_1.default.get(resultUrl, { headers: { Authorization: `Key ${falKey}` }, timeout: 15_000 });
...
async function runwayGenerate(imageUrl, prompt, duration, token, maxMs = 240_000) {
    const headers = {
        Authorization: `Bearer ${token}`,
    ...
    let submitResp;
    try {
        submitResp = await axios_1.default.post('https://api.dev.runwayml.com/v1/image_to_video', payload, { headers, timeout: 30_000 });
    }
    catch (err) {
        if (err.response) {
            ...
            if (status === 400)
                throw new Error(`Runway a rejeté la requête (400): ${errMsg}`);
            if (status === 401 || status === 403)
                throw new Error(`Runway: auth rejetée (${status})`);
    ...
    while (Date.now() - start < maxMs) {
        ...
        const statusResp = await axios_1.default.get(`https://api.dev.runwayml.com/v1/tasks/${taskId}`, { headers, timeout: 15_000, validateStatus: () => true });
        const httpStatus = statusResp.status;
        if (httpStatus === 401 || httpStatus === 403)
            throw new Error(`Runway: auth rejetée (${httpStatus})`);
        if (httpStatus === 404)
            throw new Error(`Runway: task introuvable (${taskId})`);
        if (httpStatus >= 500)
            throw new Error(`Runway: erreur serveur (${httpStatus})`);
...
function detectSceneTransformation(userPrompt) {
...
async function prepareSourceImage(imageUrl) {
    try {
        const resp = await axios_1.default.head(imageUrl, { timeout: 8_000, validateStatus: () => true });
        if (resp.status >= 400)
            return { valid: false, sizeKb: 0, note: `HTTP ${resp.status}` };
...
async function extractCarFromBackground(imageUrl, falKey, maxMs = 90_000) {
    console.log(`[extractCarFromBackground] BiRefNet — imageUrl=${imageUrl}`);
    const url = await falGenerate('fal-ai/birefnet', { image_url: imageUrl, model: 'General Use (Light)' }, falKey, maxMs);
    if (url.startsWith('{'))
        throw new Error(`BiRefNet returned unexpected JSON: ${url.slice(0, 120)}`);
...
async function generateCarInNewScene(carPngUrl, requestedScene, _carName, // kept for API compatibility — BRIA infers car from image
falKey, maxMs = 120_000) {
    ...
    try {
        ...
        if (url.startsWith('{'))
            throw new Error(`JSON inattendu: ${url.slice(0, 200)}`);
...
async function validateTransformedKeyframe(imageUrl) {
    try {
        const resp = await axios_1.default.get(imageUrl, { responseType: 'arraybuffer', timeout: 30_000 });
...
async function generateVehicleVideo(opts) {
    ...
    if (forceProvider === 'runway') {
        if (!runwayKey)
            throw new Error('RUNWAY_API_KEY non configurée dans Railway. Ajoute-la dans Railway → Variables → RUNWAY_API_KEY.');
        if (!imageUrl)
            throw new Error('Runway image-to-video nécessite une image source — voiture non trouvée dans la flotte ou sans photo.');
    ...
    if (forceProvider === 'kling') {
        if (!falKey)
            throw new Error('FAL_KEY non configurée — impossible d\'utiliser Kling.');
        if (imageUrl) {
            const url = await falGenerate('fal-ai/kling-video/v1.6/standard/image-to-video', { image_url: imageUrl, prompt, negative_prompt: negativePrompt, cfg_scale: cfgScale, duration: String(duration), aspect_ratio: '9:16' }, falKey, 240_000);
            return { url, provider: 'Kling 1.6', mode: 'image-to-video' };
        }
        const url = await falGenerate('fal-ai/kling-video/v1.6/standard/text-to-video', { prompt, negative_prompt: negativePrompt, duration: String(duration), aspect_ratio: '9:16' }, falKey, 240_000);
...
async function generateImageTool(input, sessionId) {
    const token = env_js_1.env.REPLICATE_API_TOKEN;
    if (!token)
        return '❌ REPLICATE_API_TOKEN non configuré dans Railway. Ajoute-le dans Railway → Variables.';
    ...
    await (0, telegram_js_1.sendMessage)(chatId, `🎨 *Génération image IA — Flux.1*\n_"${prompt.slice(0, 80)}"_\n⏳ 15-30 secondes...`);
    const imageUrl = await replicateGenerate('black-forest-labs/flux-1.1-pro', {
        prompt: fullPrompt,
    ...
    let delivered = false;
    try {
        await (0, telegram_js_1.sendPhoto)(chatId, imageUrl, `🎨 *Image générée — Flux.1 Pro*\n_${prompt.slice(0, 100)}_`);
        delivered = true;
    }
    catch (err) {
...
async function generateAiVideoTool(input, sessionId) {
    const falKey = env_js_1.env.FAL_KEY;
    const runwayKey = env_js_1.env.RUNWAY_API_KEY;
    ...
    if (!falKey && !runwayKey)
        return '❌ Aucun provider vidéo configuré. Ajoute FAL_KEY dans Railway → Variables.';
    const prompt = input['prompt'];
    const duration = Number(input['duration'] ?? 5);
    const carName = input['car_name'];
    const forceProvider = input['provider'] ?? 'auto';
    const chatId = chatIdFromSession(sessionId);
    // ── Duplicate lock ─────────────────────────────────────────────────────────
    ...
    try {
        ...
        const directImageUrl = input['image_url'];
        if (directImageUrl) {
        ...
        let effectiveImageUrl = carImageUrl;
        ...
        if (forceProvider !== 'auto' && sceneTransformation && carImageUrl) {
            ...
            try {
                ...
                await (0, telegram_js_1.sendMessage)(chatId, `⚠️ *Extraction fond échouée — on anime directement la photo originale.*\n_${extractErr.message.slice(0, 120)}_`);
                ...
                await (0, telegram_js_1.sendMessage)(chatId, `🎬 *Génération vidéo depuis la photo originale...*\n⏳ 60-240 secondes...`);
                ...
                const respFb = await axios_1.default.get(fbUrl, { responseType: 'arraybuffer', timeout: 60_000 });
                const bufferFb = Buffer.from(respFb.data);
                if (!(0, create_marketing_video_js_1.isValidMp4Buffer)(bufferFb))
                    throw new Error(`${fbProvider} a retourné un fichier invalide`);
                const captionFb = `🎬 *${carDisplayName} — Vidéo IA — ${fbProvider}*\n_Photo originale (extraction fond échouée)_`;
                try {
                    await (0, telegram_js_1.sendVideoBuffer)(chatId, bufferFb, captionFb);
                }
                catch {
                    await (0, telegram_js_1.sendMessage)(chatId, `${captionFb}\n\n⚠️ Envoi direct impossible.\n[Télécharger](${fbUrl})`);
                }
            ...
            await (0, telegram_js_1.sendMessage)(chatId, `✅ *Voiture extraite.* Étape 2/3 : Création de la nouvelle scène...\n_"${requestedScene.slice(0, 60)}"_\n⏳ 30-90 secondes...`);
            let transformedUrl;
            try {
                ...
                await (0, telegram_js_1.sendMessage)(chatId, `⚠️ *Création nouvelle scène échouée — on anime la photo originale.*\n_${sceneErr.message.slice(0, 120)}_`);
                ...
                const respFb2 = await axios_1.default.get(fbUrl2, { responseType: 'arraybuffer', timeout: 60_000 });
                const bufferFb2 = Buffer.from(respFb2.data);
                if (!(0, create_marketing_video_js_1.isValidMp4Buffer)(bufferFb2))
                    throw new Error(`${fbProv2} a retourné un fichier invalide`);
                const captionFb2 = `🎬 *${carDisplayName} — Vidéo IA — ${fbProv2}*\n_Photo originale (transformation scène échouée)_`;
                try {
                    await (0, telegram_js_1.sendVideoBuffer)(chatId, bufferFb2, captionFb2);
                }
                catch {
                    await (0, telegram_js_1.sendMessage)(chatId, `${captionFb2}\n\n⚠️ Envoi direct impossible.\n[Télécharger](${fbUrl2})`);
                }
            ...
            if (!kfValidation.valid) {
                await (0, telegram_js_1.sendMessage)(chatId, `❌ *Image intermédiaire invalide* (${kfValidation.reason}).\nGénération vidéo annulée — la scène créée n'est pas exploitable.`);
        ...
        const resp = await axios_1.default.get(videoUrl, { responseType: 'arraybuffer', timeout: 120_000 });
        const buffer = Buffer.from(resp.data);
        const sizeKb = Math.round(buffer.length / 1024);
        console.log(`[generateAiVideoTool] vidéo téléchargée: ${sizeKb} KB`);
        if (!(0, create_marketing_video_js_1.isValidMp4Buffer)(buffer)) {
        ...
        if (sizeKb > 45_000) {
            ...
            try {
                await (0, telegram_js_1.sendVideoBuffer)(chatId, buffer, caption);
                delivered = true;
            }
            catch (err) {
        ...
        if (delivered)
            return `✅ Vidéo réaliste créée (${provider}, ${mode}) depuis ${modeLabel} — envoyée sur Telegram ↑`;
...
async function animateCarPhotoTool(input, sessionId) {
    const falKey = env_js_1.env.FAL_KEY;
    const runwayKey = env_js_1.env.RUNWAY_API_KEY;
    if (!falKey && !runwayKey)
        return '❌ Aucun provider vidéo configuré. Ajoute FAL_KEY dans Railway → Variables.';
    const chatId = chatIdFromSession(sessionId);
    const carName = input['car_name'];
    const motionPrompt = input['motion_prompt'] ?? 'car moving forward smoothly, cinematic camera pan, golden hour lighting';
    const forceProvider = input['provider'] ?? 'auto';
    ...
    try {
        if (!imageUrl) {
            if (carName) {
                ...
                const { data: cars } = await supabase_js_1.supabase.from('cars').select('id, name, image_url').eq('available', true);
                const car = (cars ?? []).find((c) => c.image_url);
                if (!car?.image_url)
        ...
        if (forceProvider === 'runway') {
            await (0, telegram_js_1.sendMessage)(chatId, `🎬 *Animation photo — Runway Gen-3* (mode forcé)\n✅ Photo réelle de *${displayName}* trouvée.\n_Génération en mode fidélité stricte avec Runway..._\n⏳ 60-240 secondes...`);
        }
        else {
            const providerLabel = (forceProvider === 'kling' || !runwayKey) ? 'Kling 1.6' : 'Runway Gen-3';
            await (0, telegram_js_1.sendMessage)(chatId, `🎬 *Animation photo IA — ${providerLabel}*\n✅ Photo réelle de *${displayName}* trouvée.\n_Génération réaliste en cours avec ${providerLabel}..._\n⏳ 60-240 secondes...`);
        }
        const genStart = Date.now();
        const { url: videoUrl, provider, mode } = await generateVehicleVideo({
        ...
        const resp = await axios_1.default.get(videoUrl, { responseType: 'arraybuffer', timeout: 60_000 });
        const buffer = Buffer.from(resp.data);
        if (!(0, create_marketing_video_js_1.isValidMp4Buffer)(buffer)) {
        ...
        if (delivered)
            return `✅ Photo de ${displayName} animée (${provider}, ${mode}) et envoyée sur Telegram ↑`;
...
// ─── NEXUS PC AGENT ──────────────────────────────────────────────────────────
async function pingNexusTool() {
    const { isNexusOnline, pingNexus } = await Promise.resolve().then(() => __importStar(require('../actions/handlers/nexus-relay.js')));
    if (!isNexusOnline()) {
...
async function sendNexusCommandTool(input) {
    const command = input['command']?.trim();
    if (!command)
        return '❌ Commande requise (ex: "ouvre spotify", "screenshot")';
...
async function wakeNexusTool() {
    const { isNexusOnline, isLauncherOnline, wakeNexus } = await Promise.resolve().then(() => __importStar(require('../actions/handlers/nexus-relay.js')));
    if (isNexusOnline()) {
...
async function nexusFullStatusTool() {
    const { isNexusOnline, isLauncherOnline, getNexusMac, getNexusIp, getLauncherStatus } = await Promise.resolve().then(() => __importStar(require('../actions/handlers/nexus-relay.js')));
    const lines = ['📊 **État système NEXUS**\n'];
    ...
    if (launcherOk) {
        try {
            ...
            if (status['hostname'])
                lines.push(`💻 Hostname:    ${status['hostname']}`);
            if (status['uptime'])
                lines.push(`⏱️ Uptime:      ${status['uptime']}`);
            if (status['last_wake'])
                lines.push(`⏰ Dernier réveil: ${status['last_wake']}`);
            if (status['last_error'])
                lines.push(`⚠️ Dernière erreur: ${status['last_error']}`);
        }
        catch {
...
async function updateCalendarEventTool(input) {
    const googleEventId = input['google_event_id']?.trim();
    if (!googleEventId)
        return '❌ google_event_id requis — appelle list_calendar_events pour obtenir l\'ID';
    const updates = {};
    if (input['summary'])
        updates.summary = input['summary'];
    if (input['start_date'])
        updates.startDate = input['start_date'];
    if (input['end_date'])
        updates.endDate = input['end_date'];
    if (input['description'])
        updates.description = input['description'];
    if (!Object.keys(updates).length)
        return '❌ Au moins un champ à modifier requis (summary, start_date, end_date, description)';
    const ok = await (0, google_calendar_js_1.updateCalendarEvent)(googleEventId, updates);
    if (!ok)
        return `❌ Impossible de modifier l'événement ${googleEventId} — vérifie GOOGLE_SERVICE_ACCOUNT_JSON`;
    return `✅ Événement ${googleEventId} modifié dans Google Agenda!\n${Object.entries(updates).map(([k, v]) => `  ${k}: ${v}`).join('\n')}`;
}
// ─── HEALTH CHECK COMPLET ────────────────────────────────────────────────────
async function healthCheckAllTool() {
    const results = [];
    results.push('✅ Railway backend — en ligne (tu parles avec moi)');
    if (env_js_1.env.ANTHROPIC_API_KEY)
        results.push('✅ Claude API (Anthropic) — clé configurée');
    else
        results.push('❌ Claude API — ANTHROPIC_API_KEY manquant');
    if (env_js_1.env.ELEVENLABS_API_KEY)
        results.push('✅ ElevenLabs (TTS) — clé configurée');
    else
        results.push('⚠️ ElevenLabs — ELEVENLABS_API_KEY manquant (voix désactivée)');
    try {
        const { error } = await supabase_js_1.supabase.from('bookings').select('id').limit(1);
        if (error)
            results.push(`❌ Supabase — ${error.message}`);
        else
            results.push('✅ Supabase (base de données) — connexion OK');
    }
    catch (e) {
        results.push(`❌ Supabase — ${e instanceof Error ? e.message : String(e)}`);
    }
    try {
        const events = await (0, google_calendar_js_1.listUpcomingEvents)(1);
        if (events === null)
            results.push('❌ Google Calendar — token invalide ou GOOGLE_SERVICE_ACCOUNT_JSON manquant');
        else
            results.push('✅ Google Calendar — connexion OK');
    }
    catch (e) {
        results.push(`❌ Google Calendar — ${e instanceof Error ? e.message : String(e)}`);
    }
    if (env_js_1.env.TELEGRAM_BOT_TOKEN && env_js_1.env.TELEGRAM_CHAT_ID)
        results.push('✅ Telegram — configuré');
    else
        results.push('⚠️ Telegram — TELEGRAM_BOT_TOKEN ou TELEGRAM_CHAT_ID manquant');
    if (env_js_1.env.GITHUB_TOKEN)
        results.push('✅ GitHub — token présent (coder activé)');
    else
        results.push('⚠️ GitHub — GITHUB_TOKEN manquant (coder sans GitHub impossible)');
    const { isNexusOnline } = await Promise.resolve().then(() => __importStar(require('../actions/handlers/nexus-relay.js')));
    if (isNexusOnline())
        results.push('✅ NEXUS — connecté (PC en ligne)');
    else
        results.push('❌ NEXUS — hors ligne (lance start.bat sur le PC)');
