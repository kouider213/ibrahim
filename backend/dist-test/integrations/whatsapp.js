...
exports.getClientSystemPrompt = getClientSystemPrompt;
exports.isBookingRequest = isBookingRequest;
exports.isComplaint = isComplaint;
exports.sendWhatsApp = sendWhatsApp;
exports.sendBookingConfirmation = sendBookingConfirmation;
exports.send24hReminder = send24hReminder;
exports.sendReturnReminder = sendReturnReminder;
...
function detectLanguage(text) {
    ...
    if (/\b(hi|hello|hey|i |my |please|book|want|need|the |is |are |have)\b/i.test(text))
        return 'en';
    return 'fr';
}
// ── Client system prompts ─────────────────────────────────────
function getClientSystemPrompt(lang) {
...
async function sendWhatsApp(to, body) {
    ...
    try {
        const url = `https://api.twilio.com/2010-04-01/Accounts/${env_js_1.env.TWILIO_ACCOUNT_SID}/Messages.json`;
        await axios_1.default.post(url, new URLSearchParams({
            From: env_js_1.env.TWILIO_WHATSAPP_FROM,
            To: formattedTo,
            Body: body,
        }), {
            auth: { username: env_js_1.env.TWILIO_ACCOUNT_SID, password: env_js_1.env.TWILIO_AUTH_TOKEN },
            timeout: 10_000,
        });
...
async function sendBookingConfirmation(phone, clientName, carName, startDate, endDate, totalPrice, lang = 'fr') {
    let msg;
    if (lang === 'ar') {
...
// ── Send 24h reminder ─────────────────────────────────────────
async function send24hReminder(phone, clientName, carName, startDate, lang = 'fr') {
    let msg;
    if (lang === 'ar') {
...
async function sendReturnReminder(phone, clientName, carName, endDate, lang = 'fr') {
    let msg;
    if (lang === 'ar') {
