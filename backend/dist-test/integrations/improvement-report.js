...
exports.getEvolutionReport = getEvolutionReport;
exports.formatReportForKouider = formatReportForKouider;
...
async function generateMonthlyReport(year, month) {
    ...
    const { data: newRules } = await supabase_js_1.supabase
        .from('Dzaryx_rules')
        .select('id')
    ...
    const { data: monthFeedback } = await supabase_js_1.supabase
        .from('Dzaryx_feedback')
        .select('rating, action_type, comment')
        .gte('created_at', startDate)
        .lt('created_at', endDate);
    const feedbacks = (monthFeedback ?? []);
    ...
    const patterns = await (0, feedback_system_js_1.getLearningPatterns)();
    ...
    feedbacks.forEach(f => {
        ...
        if (f.rating === 'positive')
            performanceByCategory[f.action_type].positive++;
        if (f.rating === 'negative')
            performanceByCategory[f.action_type].negative++;
    });
    ...
    if (positiveFeedbackRate < 0.7) {
        recommendations.push('⚠️ Taux de satisfaction faible (<70%) — analyser les feedbacks négatifs');
    }
    Object.entries(performanceByCategory).forEach(([cat, perf]) => {
        if (perf.success_rate < 0.6) {
            recommendations.push(`⚠️ Catégorie "${cat}" sous-performante (${Math.round(perf.success_rate * 100)}%) — revoir la stratégie`);
        }
    });
    if (newRulesCount === 0) {
...
async function getEvolutionReport(months = 6) {
    const now = new Date();
    const evolution = [];
    ...
    const avgPositiveRate = evolution.reduce((sum, e) => sum + e.positive_rate, 0) / evolution.length;
    const isImproving = evolution.length >= 2 &&
...
function formatReportForKouider(report) {
...
//# sourceMappingURL=improvement-report.js.map