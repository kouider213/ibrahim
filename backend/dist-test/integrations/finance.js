...
exports.seedPricingTable = seedPricingTable;
exports.computeBookingFinancials = computeBookingFinancials;
exports.getFinancialReport = getFinancialReport;
exports.formatFinancialReport = formatFinancialReport;
const supabase_js_1 = require("./supabase.js");
const pricing_js_1 = require("../config/pricing.js");
// ── Seed pricing table ────────────────────────────────────────────────────────
async function seedPricingTable() {
    const rows = pricing_js_1.VEHICLE_PRICING.map(p => ({
        vehicle_name: p.name,
    ...
    if (error)
        console.warn('[finance] Pricing seed failed:', error.message);
    else
        console.log('[finance] Pricing table seeded:', rows.length, 'vehicles');
}
...
// ── Financial report ──────────────────────────────────────────────────────────
async function getFinancialReport(year, month) {
    let startDate;
    let endDate;
    let period;
    if (month) {
...
// ── Formatted report ──────────────────────────────────────────────────────────
function formatFinancialReport(report) {
    const lines = [
    ...
    if (report.missingOwnerPrice > 0) {
        lines.push(``, `⚠️ DONNÉES MANQUANTES: ${report.missingOwnerPrice} réservation(s) sans owner_price_per_day`, `   → bénéfice partiel (les réservations sans prix Houari sont exclues du calcul)`, `   → renseignez owner_price_per_day dans Supabase pour un calcul complet`);
    }
    if (report.missingClientPrice > 0) {
        lines.push(``, `❌ PRIX CLIENT MANQUANT: ${report.missingClientPrice} réservation(s) sans client_price_per_day ni final_price`, `   → Impossible de calculer sans données financières réelles`);
    ...
    for (const b of report.bookings) {
        ...
        lines.push(`- ${b.client_name} | ${b.car_name} | ${b.nb_days}j` +
