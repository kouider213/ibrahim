...
exports.getLatestDeploymentStatus = getLatestDeploymentStatus;
exports.waitForDeploy = waitForDeploy;
exports.getRailwayLogs = getRailwayLogs;
const axios_1 = __importDefault(require("axios"));
const env_js_1 = require("../config/env.js");
const RAILWAY_GQL = 'https://backboard.railway.app/graphql/v2';
async function railwayQuery(query, variables = {}) {
    const token = env_js_1.env.RAILWAY_TOKEN;
    if (!token)
        throw new Error('RAILWAY_TOKEN non configuré dans Railway Variables');
    const { data } = await axios_1.default.post(RAILWAY_GQL, { query, variables }, {
...
async function getLatestDeploymentStatus() {
    const projectId = env_js_1.env.RAILWAY_PROJECT_ID;
    const serviceId = env_js_1.env.RAILWAY_SERVICE_ID;
    if (!env_js_1.env.RAILWAY_TOKEN || !projectId || !serviceId)
        return null;
    try {
...
async function waitForDeploy(timeoutMs = 180_000) {
    if (!env_js_1.env.RAILWAY_TOKEN)
        return '⚠️ RAILWAY_TOKEN non configuré.';
    ...
    while (Date.now() - start < 30_000) {
        ...
        if (dep && dep.id !== targetId) {
            targetId = dep.id;
            break;
        }
    }
    if (!targetId)
        return '⚠️ Aucun déploiement détecté après 30s. Vérifie que le push GitHub a bien déclenché Railway.';
...
async function getRailwayLogs(limit = 50) {
    const projectId = env_js_1.env.RAILWAY_PROJECT_ID;
    const serviceId = env_js_1.env.RAILWAY_SERVICE_ID;
    if (!env_js_1.env.RAILWAY_TOKEN) {
    ...
    `, { projectId, serviceId });
        ...
        if (!latest)
            return 'Aucun déploiement trouvé pour ce service.';
