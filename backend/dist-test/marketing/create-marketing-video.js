...
exports.isValidMp4Buffer = isValidMp4Buffer;
exports.mergeVideoWithAudio = mergeVideoWithAudio;
exports.executeCreateMarketingVideo = executeCreateMarketingVideo;
...
const approval_store_js_1 = require("./approval-store.js");
// ─── Constantes ───────────────────────────────────────────────
 ...
 */
async function mergeVideoWithAudio(videoBuffer, audioBuffer) {
...
async function downloadBuffer(url, timeout = 30_000) {
    const resp = await axios_1.default.get(url, { responseType: 'arraybuffer', timeout });
...
async function downloadPexelsBg(keyword, destPath) {
...
function buildVideo1080(params) {
    ...
    return new Promise((resolve, reject) => {
        ...
        args.push('-c:v', 'libx264', '-preset', 'fast', '-crf', '22', '-profile:v', 'high', '-level', '4.1', '-pix_fmt', 'yuv420p', '-r', '25', '-movflags', '+faststart', outputPath);
...
function buildVideoSimple(carImagePath, audioPath, outputPath, bgPath) {
    return new Promise((resolve, reject) => {
        const bin = ffmpeg_static_1.default;
        if (!bin) {
            reject(new Error('ffmpeg-static not found'));
        ...
        if (bgPath) {
            ...
            if (audioPath)
                args.push('-i', audioPath);
            args.push('-filter_complex', `[0:v]scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H}[bg];` +
                `[1:v]scale=${W - 100}:-1[car];[bg][car]overlay=(W-w)/2:(H-h)*0.42[out]`, '-map', '[out]');
            if (audioPath)
            ...
            if (audioPath)
                args.push('-i', audioPath);
            args.push('-vf', `scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},eq=saturation=1.5:contrast=1.12`);
            if (audioPath)
...
async function uploadToSupabaseVideos(buffer, filename) {
    const { error } = await supabase_js_1.supabase.storage
        .from('videos')
    ...
    const { data: publicData } = supabase_js_1.supabase.storage
        .from('videos')
        .getPublicUrl(filename);
    return publicData.publicUrl;
}
// ─── Fonction principale exportée ─────────────────────────────
async function executeCreateMarketingVideo(input, chatId) {
    ...
    const cars = (carsRaw ?? []);
    if (!cars.length)
        throw new Error('Aucune voiture disponible dans la flotte.');
    const carsWithImage = cars.filter(c => c.image_url);
    if (!carsWithImage.length)
        throw new Error('Aucune voiture avec photo — ajoute des images dans le tableau de bord.');
...
RÉPONDS UNIQUEMENT avec le script, sans guillemets ni commentaires.`,
    ...
    await (0, telegram_js_1.sendMessage)(chatId, `🎬 *Vidéo marketing — ${car.name}*\n_FFmpeg HD 1080×1920${backgroundEffect ? ` · fond ${backgroundEffect}` : ''}_\n⏳ Montage en cours...`).catch(() => { });
    ...
    if (videoBuffer) {
        try {
            ...
            if (imgBuf) {
                try {
                    await (0, telegram_js_1.sendPhotoBuffer)(chatId, imgBuf, approvalMsg);
                    telegramDelivered = true;
                }
                catch (photoErr) {
        ...
        const voiceBuffer = await (0, dispatcher_js_1.synthesizeVoice)(script).catch(() => null);
        if (voiceBuffer) {
            await (0, telegram_js_1.sendVoiceBuffer)(chatId, voiceBuffer).catch(() => { });
        }
    }
    else {
...
//# sourceMappingURL=create-marketing-video.js.map