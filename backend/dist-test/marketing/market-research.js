...
exports.runTikTokMarketResearch = runTikTokMarketResearch;
...
function getSeasonalContext(month) {
    if (month >= 6 && month <= 8)
        return 'Saison MRE — forte demande, familles diaspora, tourisme estival Oran';
    if (month === 3 || month === 4)
        return 'Ramadan — location nocturne populaire, sorties après iftar, famille étendue';
    if (month === 12 || month === 1)
        return 'Hiver — longue durée, voyages familiaux, fêtes de fin d\'année';
    if (month === 5)
        return 'Pré-saison — anticiper les vacances d\'été, réservations anticipées';
    if (month === 9 || month === 10)
        return 'Rentrée — déplacements professionnels, étudiants, business';
    return 'Période standard — clients locaux professionnels et loisirs';
}
// ── Claude prompts ─────────────────────────────────────────────────────────────
function buildPromptWithData(carList, season, week, formattedData) {
...
function buildPromptNoData(carList, season, week, error) {
    return `Tu es un expert marketing TikTok spécialisé location de voitures en Algérie.
...
// ── Main export ───────────────────────────────────────────────────────────────
async function runTikTokMarketResearch(cars, carFocus, extraHashtags) {
    ...
    let tiktokData;
    try {
        tiktokData = await (0, apify_tiktok_js_1.scrapeTikTokForOranCars)(carFocus, extraHashtags);
    }
    catch (err) {
        tiktokData = {
            source: 'unavailable',
    ...
    const dataQuality = !hasRealData
        ? 'no_data'
        : tiktokData.videos.length >= 10 ? 'real' : 'partial';
    // ── Step 2: Build prompt using real data only ────────────────────────────────
    const formattedData = (0, apify_tiktok_js_1.formatTikTokDataForReport)(tiktokData);
    ...
    const response = await (0, claude_api_js_1.chat)([{ role: 'user', content: prompt }]);
    let parsed = {};
    try {
    ...
    return {
        ...
        real_metrics: hasRealData ? {
            ...
            top_authors: tiktokData.topAuthors.map(a => ({
                handle: a.handle,
        ...
        raw_json: (0, apify_tiktok_js_1.serializeTikTokData)(tiktokData),
    };
}
