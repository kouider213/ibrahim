...
exports.autonomousFixTypeScript = autonomousFixTypeScript;
exports.runAutonomousTask = runAutonomousTask;
...
async function autonomousFixTypeScript(projectToken) {
    const jobId = `auto_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
    const start = Date.now();
    const proj = (0, nexus_environment_js_1.resolveProject)(projectToken);
    if (!proj)
        throw new Error(`Unknown project: "${projectToken}"`);
    const steps = [];
    ...
    const tsc1 = await step('tsc_initial', async () => {
        const r = await (0, nexus_relay_js_1.nexusTerminalRun)('npx tsc --noEmit 2>&1', proj.key, undefined, 60);
    ...
    (0, nexus_relay_js_1.sendToNexus)('nexus:autonomous_start', { jobId, project: proj.key, errors: errorCount });
    // ── Step 2: Claude Code fix ───────────────────────────────────────────────
    const prompt = `Corrige toutes les erreurs TypeScript suivantes dans ce projet. ` +
    ...
    await step('claude_code_fix', async () => {
        const r = await (0, nexus_relay_js_1.nexusClaudeCodeStart)(proj.key, prompt, 180);
    ...
    if (tsc2.ok) {
        const commit = await step('git_commit', async () => {
            const r = await (0, nexus_relay_js_1.nexusTerminalRun)(`git add -A && git commit -m "fix: TypeScript errors auto-fixed by Nexus autonomous mode"`, proj.key, undefined, 30);
...
async function runAutonomousTask(task, project) {
    switch (task) {
        case 'fix_typescript': return autonomousFixTypeScript(project);
        default: throw new Error(`Unknown autonomous task: "${task}"`);
    }
}
//# sourceMappingURL=nexus-autonomous.js.map