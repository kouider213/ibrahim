...
exports.getMonthlyImprovementReport = getMonthlyImprovementReport;
exports.getLearningEvolution = getLearningEvolution;
exports.getKouiderPreferences = getKouiderPreferences;
exports.handleLearning = handleLearning;
...
// ─────────────────────────────────────────────────────────────────
async function getMonthlyImprovementReport(args) {
    try {
        const now = new Date();
        ...
        const { data: feedbacks, error: feedbackError } = await supabase_js_1.supabase
            .from('Dzaryx_feedback')
            .select('*')
        ...
        const { data: rules, error: rulesError } = await supabase_js_1.supabase
            .from('Dzaryx_rules')
            .select('*')
            .gte('created_at', startDate.toISOString())
            .lte('created_at', endDate.toISOString());
        ...
        feedbacks?.forEach((fb) => {
            ...
            byType[fb.action_type][fb.rating]++;
        });
    ...
    try {
        ...
        feedbacks?.forEach((fb) => {
            ...
            if (fb.rating === 'positive')
                monthlyData[key].positive++;
            if (fb.rating === 'negative')
                monthlyData[key].negative++;
        });
        ...
        return {
            ...
            avg_satisfaction: Math.round(evolution.reduce((sum, m) => sum + m.satisfaction_rate, 0) / evolution.length) || 0,
        };
    }
    catch (error) {
        return {
...
async function updateMonthlyStats(year, month) {
    const startDate = new Date(year, month - 1, 1);
    const endDate = new Date(year, month, 0, 23, 59, 59);
    const { data: feedbacks } = await supabase_js_1.supabase
        .from('Dzaryx_feedback')
        .select('rating')
        .gte('created_at', startDate.toISOString())
        .lte('created_at', endDate.toISOString());
    const { data: rules } = await supabase_js_1.supabase
        .from('Dzaryx_rules')
        .select('id')
        .gte('created_at', startDate.toISOString())
        .lte('created_at', endDate.toISOString());
...
function generateRecommendations(byType, negativeCount, totalCount) {
...
function analyzeResponseStyle(feedbacks) {
    ...
    if (shortPreferred)
        return 'short';
    if (detailedPreferred)
        return 'detailed';
    return 'balanced';
}
function analyzeTonePreference(feedbacks) {
    ...
    if (friendlyPreferred)
        return 'friendly';
    if (professionalPreferred)
        return 'professional';
    return 'friendly';
}
function analyzeTikTokFavorites(feedbacks) {
...
function analyzeBookingPreferences(feedbacks) {
...
async function handleLearning(payload) {
    try {
        let data;
        switch (payload.action) {
            case 'record_feedback':
                data = await recordFeedback(payload.params);
                break;
            case 'get_monthly_improvement_report':
                data = await getMonthlyImprovementReport(payload.params);
                break;
            case 'get_learning_evolution':
                data = await getLearningEvolution(payload.params);
                break;
