...
let fleetCache = null;
async function getLiveFleetContext() {
    if (fleetCache && Date.now() - fleetCache.ts < 3 * 60 * 1000)
        return fleetCache.data;
    try {
        const today = new Date().toISOString().slice(0, 10);
        const [{ data: cars }, { data: bookings }] = await Promise.all([
            supabase_js_1.supabase.from('cars').select('name, category, resale_price, available').order('name'),
        ...
        const rentedCarIds = new Set((bookings ?? []).map((b) => b.car_id));
        const fleetLines = (cars ?? []).map((c) => {
            const rented = rentedCarIds.has(c.id) || !c.available;
...
router.post('/chat', async (req, res) => {
    ...
    try {
        ...
        if ((0, llm_router_js_1.isGroqAvailable)()) {
            try {
                text = await (0, llm_router_js_1.callGroq)(lastUserMsg, systemPrompt);
                console.log('[AI_ROUTER] provider=groq route=widget');
            }
            catch { }
        }
        if (!text && (0, llm_router_js_1.isGeminiAvailable)()) {
            try {
                text = await (0, llm_router_js_1.callGemini)(lastUserMsg, systemPrompt);
                console.log('[AI_ROUTER] provider=gemini route=widget');
            }
            catch { }
        }
        if (!text && (0, llm_router_js_1.isOpenAIAvailable)()) {
            try {
                text = await (0, llm_router_js_1.callOpenAI)(messages, systemPrompt);
...
router.get('/embed.js', (_req, res) => {
    const backendUrl = env_js_1.env.BACKEND_URL;
...
function buildWidgetScript(backendUrl) {
    return `
(function() {
  'use strict';
