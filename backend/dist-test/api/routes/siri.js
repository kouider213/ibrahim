...
const sirisSchema = zod_1.z.object({
    message: zod_1.z.string().min(1).max(4000),
    sessionId: zod_1.z.string().min(1).max(128).default('siri-shortcut'),
});
...
router.post('/', async (req, res) => {
    const token = req.headers['x-Dzaryx-token'];
    if (token !== env_js_1.env.MOBILE_ACCESS_TOKEN) {
    ...
    try {
        const response = await (0, orchestrator_js_1.processMessage)(parsed.data.message, parsed.data.sessionId);
