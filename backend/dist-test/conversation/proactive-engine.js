...
exports.clearAllLocks = clearAllLocks;
exports.runProactiveEngine = runProactiveEngine;
...
async function acquireDailyLock(name) {
    const today = new Date().toISOString().slice(0, 10);
    const result = await queue_js_1.redis.set(`p12c:${name}:${today}`, '1', 'EX', 86400, 'NX');
    return result === 'OK';
}
// Redis weekly lock — keyed on ISO week Monday
async function acquireWeeklyLock(name) {
    const now = new Date();
    const day = now.getDay();
    const diff = now.getDate() - day + (day === 0 ? -6 : 1);
    const monday = new Date(now.getFullYear(), now.getMonth(), diff).toISOString().slice(0, 10);
    const result = await queue_js_1.redis.set(`p12c:${name}:week:${monday}`, '1', 'EX', 604800, 'NX');
    return result === 'OK';
}
async function clearAllLocks() {
    ...
    const diff = now.getDate() - day + (day === 0 ? -6 : 1);
    const monday = new Date(now.getFullYear(), now.getMonth(), diff).toISOString().slice(0, 10);
    await queue_js_1.redis.del(`p12c:pre-work:${today}`, `p12c:after-work:${today}`, `p12c:heat-alert:${today}`, `p12c:family-time:${today}`, `p12c:unpaid-reminder:${today}`, `p12c:late-return:${today}`, `p12c:booking-tomorrow:${today}`, `p12c:calendar-appt:${today}`, `p12c:personal-schedule:${today}`, `p12c:tiktok-reminder:week:${monday}`);
}
...
async function triggerPreWorkReminder(workDays, workStart, commute, tz, demo) {
...
async function triggerAfterWorkRest(workDays, workEnd, tz, demo) {
    const { hour, minute, dayOfWeek } = getLocalTime(tz);
    if (!demo && !workDays.includes(dayOfWeek)) {
...
async function triggerHeatAlert(demo) {
    ...
    let weather;
    try {
        weather = await (0, web_search_js_1.getOranWeather)();
    }
    catch {
    ...
    if (!demo)
        await (0, pushover_js_1.notifyOwner)(`🌡️ Chaleur Oran ${weather.temperature}°C`, 'Conseille stationnement ombragé', false);
    return { trigger: 'heat-alert', status: 'SENT' };
}
// ── Trigger 4: TikTok weekly reminder ────────────────────────────
async function triggerTikTokReminder(demo) {
    ...
    const dayNames = { 0: 'dimanche', 1: 'lundi', 2: 'mardi', 3: 'mercredi', 4: 'jeudi', 5: 'vendredi', 6: 'samedi' };
    const p = demo ? '🧪 [DEMO] ' : '';
    await tg([
...
async function triggerFamilyTime(profile, tz, demo) {
...
async function triggerUnpaidReminder(demo) {
    ...
    if (withRemaining.length > 5)
        lines.push(`  … et ${withRemaining.length - 5} autre(s)`);
    await tg(lines.join('\n'));
    await (0, pushover_js_1.notifyOwner)(`💸 ${withRemaining.length} impayé(s) >7j`, `${totalDue}€ à encaisser`, false);
    return { trigger: 'unpaid-reminder', status: 'SENT' };
}
// ── Trigger 7: Late vehicle returns ──────────────────────────────
async function triggerLateReturn(demo) {
    const { hour } = getLocalTime('Africa/Algiers');
    ...
    await tg(lines.join('\n'));
    await (0, pushover_js_1.notifyOwner)(`🚨 ${overdue.length} retard(s) retour`, overdue.map(b => b.client_name).join(', '), true);
    return { trigger: 'late-return', status: 'SENT' };
}
// ── Trigger 8: Bookings tomorrow (J+1) ───────────────────────────
async function triggerBookingTomorrow(demo) {
    const { hour } = getLocalTime('Europe/Brussels');
...
async function triggerCalendarAppointment(commute, tz, demo) {
    ...
    let events = [];
    try {
        events = await (0, google_calendar_js_1.listUpcomingEvents)(5);
    }
    catch {
...
async function triggerPersonalSchedule(profile, workDays, tz, demo) {
    const { hour, minute, dayOfWeek } = getLocalTime(tz);
    // Morning 6h30–8h30
...
// ── Main orchestrator ─────────────────────────────────────────────
async function runProactiveEngine(_job, force = false, demo = false) {
    if (force) {
        await clearAllLocks();
        console.log('[p12c] Force mode — all locks cleared');
    }
    const profile = await (0, supabase_js_1.getUserProfile)().catch(() => null);
    if (!profile) {
    ...
    const wrap = (name, fn) => fn.catch(err => ({ trigger: name, status: 'ERROR', reason: err.message }));
