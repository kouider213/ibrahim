...
exports.runCodeAudit = runCodeAudit;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const env_js_1 = require("../config/env.js");
const github_js_1 = require("../integrations/github.js");
const railway_js_1 = require("../integrations/railway.js");
// ── Constants ─────────────────────────────────────────────────────────────────
...
async function execReadFile(path) {
    const result = await (0, github_js_1.getFileContent)(path, AUDIT_REPO);
    if (!result)
        return `❌ Fichier non trouvé: ${path} dans ${AUDIT_REPO}`;
    return result.content;
}
async function execApplyPatch(path, oldString, newString, commitMsg) {
    const file = await (0, github_js_1.getFileContent)(path, AUDIT_REPO);
    if (!file)
        return { result: `❌ Fichier non trouvé: ${path}` };
    ...
    const writeResult = await (0, github_js_1.updateFile)(path, newContent, msg, AUDIT_REPO);
    if (!writeResult)
        return { result: `❌ GitHub updateFile a échoué pour ${path}` };
...
async function execListDirectory(path) {
    const files = await (0, github_js_1.listDirectory)(path, AUDIT_REPO);
    if (!files.length)
        return `Répertoire vide ou non trouvé: ${path || '/'}`;
    return files.map(f => `${f.type === 'dir' ? '📁' : '📄'} ${f.path}`).join('\n');
}
async function execGetRailwayLogs(limit) {
    return (0, railway_js_1.getRailwayLogs)(limit);
}
// ── Runner ────────────────────────────────────────────────────────────────────
async function runCodeAudit(requestId, timeoutMs = 90_000) {
    const t0 = Date.now();
    ...
    const existingFile = await (0, github_js_1.getFileContent)(AUDIT_FILE, AUDIT_REPO);
    if (!existingFile) {
        return makeFailedResult(requestId, model, toolsCalled, patchesApplied, 0, 0, Date.now() - t0, `Fichier test non trouvé: ${AUDIT_FILE} — committer backend/audit-test/revenue-calc.ts dans le repo`);
    }
    ...
    if (!hasBug) {
        ...
        const restoreResult = await (0, github_js_1.updateFile)(AUDIT_FILE, BUGGY_CODE, `P8 audit: restore test bug (TS2362) for re-run`, AUDIT_REPO);
        if (!restoreResult) {
            return makeFailedResult(requestId, model, toolsCalled, patchesApplied, 0, 0, Date.now() - t0, `Impossible de restaurer le bug test dans ${AUDIT_FILE}`);
        }
    ...
    try {
        while (iterCount < MAX_ITER && (Date.now() - t0) < timeoutMs) {
            ...
            console.log(`[code-audit:${requestId}] iter=${iterCount} stop=${response.stop_reason} ` +
            ...
            for (const tb of toolUseBlocks) {
                ...
                if (blocked) {
                    ...
                    result = await execReadFile(input['path']);
                }
                else if (tb.name === 'apply_patch') {
                    const r = await execApplyPatch(input['path'], input['old_string'], input['new_string'], input['commit_msg'] ?? '');
                    result = r.result;
                    commitSha = r.commitSha;
                    if (r.patch)
                        patchesApplied.push(r.patch);
                }
                else if (tb.name === 'list_directory') {
                    result = await execListDirectory(input['path'] ?? '');
                }
                ...
                console.log(`[code-audit:${requestId}] ` +
        ...
        const finalFile = await (0, github_js_1.getFileContent)(AUDIT_FILE, AUDIT_REPO);
        ...
        const verdict = bugFixed && fixVerified && readCalls.length >= 2 ? 'VERIFIED' :
        ...
        console.log(`[code-audit:${requestId}] DONE verdict=${verdict} ` +
        ...
        try {
            const f = await (0, github_js_1.getFileContent)(AUDIT_FILE, AUDIT_REPO);
...
// ── Helper ─────────────────────────────────────────────────────────────────────
function makeFailedResult(requestId, model, toolsCalled, patchesApplied, inp, out, ms, errMsg) {
    return {
        request_id: requestId,
        agent_id: 'code_audit',
        provider: 'claude',
...
//# sourceMappingURL=code-audit-runner.js.map