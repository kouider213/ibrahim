...
exports.runCompetitorAgentWithTools = runCompetitorAgentWithTools;
...
function extractCompetitorNames(text) {
    const patterns = [
...
function classifyDataQuality(result) {
    if (!result || result.trim().length < 20)
        return 'empty';
    if (/inaccessible|blocked|erreur|timeout|no results/i.test(result))
        return 'blocked';
    if (/erreur|exception/i.test(result))
        return 'error';
    return 'real';
}
// ── Runner ─────────────────────────────────────────────────────────────────────
async function runCompetitorAgentWithTools(userMessage, requestId, timeoutMs = 120_000) {
    const t0 = Date.now();
    ...
    try {
        while (iterCount < MAX_ITER && (Date.now() - t0) < timeoutMs) {
            ...
            console.log(`[competitor-agent:${requestId}] iter=${iterCount} stop_reason=${response.stop_reason} ` +
            ...
            for (const tb of toolUseBlocks) {
                ...
                console.log(`[competitor-agent:${requestId}] ` +
                    `tool=${tb.name} query="${(input['query'] ?? '').slice(0, 50)}" ` +
        ...
        const verdict = realDataCalls.length >= 3 && finalAnalysis.length > 200 ? 'VERIFIED' :
...
//# sourceMappingURL=competitor-agent-runner.js.map