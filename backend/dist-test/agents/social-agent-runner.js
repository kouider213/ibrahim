...
exports.runSocialAgentWithTools = runSocialAgentWithTools;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const env_js_1 = require("../config/env.js");
const tool_executor_js_1 = require("../integrations/tool-executor.js");
const apify_tiktok_js_1 = require("../integrations/apify-tiktok.js");
// ── Authorized tools ──────────────────────────────────────────────────────────
...
function sanitizeForClaude(s) {
    return s.replace(/[\uD800-\uDFFF]/g, '');
}
// ── Tool executors ─────────────────────────────────────────────────────────────
async function runGetTikTokMarket() {
    try {
        const data = await (0, apify_tiktok_js_1.scrapeTikTokForOranCars)();
        const formatted = (0, apify_tiktok_js_1.formatTikTokDataForReport)(data);
...
async function runGetTikTokProfile(username) {
    ...
    try {
        const items = await (0, apify_tiktok_js_1.apifyRun)('clockworks~tiktok-scraper', {
        ...
        const videos = items.map(apify_tiktok_js_1.parseVideo);
        // Aggregate
        ...
        const lines = [
            ...
            '',
            'Top vidéos:',
            ...topVids.map((v, i) => `  ${i + 1}. "${v.description.slice(0, 80)}" — ${(v.views ?? '?').toLocaleString()} vues | ❤️ ${v.likes?.toLocaleString() ?? '?'} | eng: ${v.engagementRate ?? '?'}%`),
        ];
...
function classifyDataQuality(result, videos) {
    if (videos > 0)
        return 'real';
    if (!result || result.trim().length < 20)
        return 'no_data';
    if (/inaccessible|bloqué|blocked|0 vidéos|0 résultats/i.test(result))
        return 'no_data';
    if (/erreur|exception|error/i.test(result))
        return 'error';
...
// ── Runner ────────────────────────────────────────────────────────────────────
async function runSocialAgentWithTools(userMessage, requestId, timeoutMs = 180_000) {
    const t0 = Date.now();
    ...
    try {
        while (iterCount < MAX_ITER && (Date.now() - t0) < timeoutMs) {
            ...
            console.log(`[social-agent:${requestId}] iter=${iterCount} stop_reason=${response.stop_reason} ` +
            ...
            for (const tb of toolUseBlocks) {
                ...
                if (blocked) {
                    ...
                    if (/fik/i.test(username))
                        fik_profile = r.profile;
                }
                else {
                ...
                console.log(`[social-agent:${requestId}] ` +
                    `tool=${tb.name} ` +
        ...
        const verdict = totalTikTokVideos > 0 && finalAnalysis.length > 200 ? 'VERIFIED' :
        ...
        console.log(`[social-agent:${requestId}] DONE verdict=${verdict} ` +
...
//# sourceMappingURL=social-agent-runner.js.map