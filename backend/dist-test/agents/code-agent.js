...
exports.runCodeAgent = runCodeAgent;
...
async function runTool(name, input, chatId) {
    const repo = input['repo'] ?? 'ibrahim';
    switch (name) {
        case 'read_file': {
            const result = await (0, github_js_1.getFileContent)(input['path'], repo);
            if (!result)
                return `❌ Fichier non trouvé: ${input['path']} dans ${repo}`;
        ...
        case 'apply_patch': {
            ...
            const message = input['message'] ?? 'patch: code agent edit';
            const file = await (0, github_js_1.getFileContent)(path, repo);
            if (!file)
            ...
            await (0, telegram_js_1.sendMessage)(chatId, `🔧 _Patch → ${path}_\n\`${preview}...\``);
            return `✅ Patch appliqué dans ${path} (commit: ${writeResult.commitSha})`;
        }
        case 'create_file': {
            const path = input['path'];
            const content = input['content'];
            const message = input['message'] ?? 'feat: new file';
            const writeResult = await (0, github_js_1.updateFile)(path, content, message, repo);
            if (!writeResult)
                return `❌ Impossible de créer ${path}`;
            await (0, telegram_js_1.sendMessage)(chatId, `📄 _Créé → ${path}_`);
            return `✅ Fichier créé: ${path} (commit: ${writeResult.commitSha})`;
        }
        case 'verify_deploy': {
            await (0, telegram_js_1.sendMessage)(chatId, `⏳ _Vérification déploiement Railway..._`);
            const result = await (0, railway_js_1.waitForDeploy)(240_000);
            await (0, telegram_js_1.sendMessage)(chatId, result.startsWith('✅') ? result : `❌ ${result}`);
            return result;
        }
        default:
...
async function runCodeAgent(task, chatId, repo = 'ibrahim') {
    await (0, telegram_js_1.sendMessage)(chatId, `🤖 *Code Agent démarré*\n_Tâche: ${task.slice(0, 120)}_\n\n⏳ En cours...`);
    const messages = [
    ...
    for (let i = 0; i < MAX; i++) {
        ...
        if (response.stop_reason === 'end_turn') {
            const text = response.content
                ...
                .map(b => b.text)
                .join('');
            await (0, telegram_js_1.sendMessage)(chatId, `✅ *Code Agent terminé*\n\n${text.slice(0, 600)}`);
            return;
        }
        const toolResults = [];
        for (const block of response.content) {
            if (block.type !== 'tool_use')
                continue;
            const result = await runTool(block.name, block.input, chatId);
            toolResults.push({ type: 'tool_result', tool_use_id: block.id, content: result });
        }
        messages.push({ role: 'user', content: toolResults });
    }
    await (0, telegram_js_1.sendMessage)(chatId, `⚠️ Code Agent: limite d'itérations atteinte. Vérifie l'état du repo sur GitHub.`);
}
