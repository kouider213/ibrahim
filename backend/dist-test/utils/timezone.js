...
exports.FALLBACK_TZ = void 0;
exports.isValidTimezone = isValidTimezone;
exports.getUTCOffsetMs = getUTCOffsetMs;
exports.getUTCOffsetMinutes = getUTCOffsetMinutes;
exports.getUTCOffsetString = getUTCOffsetString;
exports.toLocalISO = toLocalISO;
exports.parseLocalHHMM = parseLocalHHMM;
exports.resolveTimezone = resolveTimezone;
exports.getTimezoneConversion = getTimezoneConversion;
exports.getServerTimezone = getServerTimezone;
exports.FALLBACK_TZ = 'Europe/Brussels';
// ── Validation ────────────────────────────────────────────────────────────────
...
function getUTCOffsetMs(tz, date = new Date()) {
    const utcStr = date.toLocaleString('en-US', { timeZone: 'UTC' });
    const tzStr = date.toLocaleString('en-US', { timeZone: tz });
    return new Date(tzStr).getTime() - new Date(utcStr).getTime();
}
function getUTCOffsetMinutes(tz, date = new Date()) {
    return Math.round(getUTCOffsetMs(tz, date) / 60_000);
}
// Returns "+02:00" / "-05:30" style string
function getUTCOffsetString(tz, date = new Date()) {
...
function parseLocalHHMM(HHmm, tz) {
...
function resolveTimezone(explicit, // from tool input
userProfile, // from Redis session (X-Timezone header)
deviceTz) {
    // 1. Explicit parameter (tool call)
    if (explicit) {
        if (isValidTimezone(explicit))
            return { timezone: explicit, source: 'explicit', valid: true };
    ...
    return { timezone: exports.FALLBACK_TZ, source: 'fallback', valid: true };
}
function getTimezoneConversion(tz, date = new Date()) {
